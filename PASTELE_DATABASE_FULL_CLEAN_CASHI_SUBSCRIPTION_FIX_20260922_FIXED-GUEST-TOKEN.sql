...
-- NOTE: Do not create a UNIQUE index on guest_access_token alone.
-- One guest token is intentionally reusable across different products.
-- The canonical per-guest/per-product uniqueness is created in FINAL GUEST ORDER FIX below.
...
-- Repair legacy duplicate rows before creating the canonical unique index.
-- We keep the newest successful order (or newest order when none is successful)
-- on the original guest token and assign a fresh token to extra duplicates.
-- No orders are deleted.
WITH ranked AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY guest_access_token, product_id, lower(coalesce(item_type,''))
      ORDER BY
        CASE
          WHEN lower(coalesce(status,'')) IN ('paid','success','completed','settled')
            THEN 0
          ELSE 1
        END,
...
SET guest_access_token = gen_random_uuid()::text
FROM ranked r
WHERE o.id = r.id
  AND r.rn > 1;
