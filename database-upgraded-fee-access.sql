...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM public.purchases WHERE order_id=o.id) THEN
    ...
    INSERT INTO public.wallet_transactions(
      ...
      'bayargg-order:'||o.id::text,'pending',available_at,'H2'
    )
    ON CONFLICT(user_id,reference) DO NOTHING;
    ...
    INSERT INTO public.transactions(
      ...
      'bayargg-order:'||o.id::text,'Creator 70% — settlement H+2 (48 jam)'
    )
    ON CONFLICT(user_id,reference) DO NOTHING;
...
/* ============================================================
   ...
   SUBSCRIPTION 1 MONTH — Rp150.000:
     ...
     - Active: 30 days
     - QRIS/Bayar.gg handled by checkout layer
...
CREATE TABLE IF NOT EXISTS public.account_plan_catalog (
  ...
  payment_provider text NOT NULL DEFAULT 'bayargg',
...
('subscription_1','Langganan 1 Hari',15000,1,2,3,false,'bayargg','qris',true),
('subscription_3','Langganan 3 Hari',30000,3,3,5,false,'bayargg','qris',true),
('subscription_7','Langganan 7 Hari',50000,7,5,7,false,'bayargg','qris',true),
('subscription_30','Langganan 1 Bulan',150000,30,10,20,false,'bayargg','qris',true),
('premium','Premium Full Access',250000,36500,NULL,NULL,true,'bayargg','qris',true)
...
BEGIN
  ...
  IF normalized LIKE 'subscription_%' AND p_days IS NOT NULL THEN
    IF p_days <> CASE normalized
      WHEN 'subscription_1' THEN 1
      WHEN 'subscription_3' THEN 3
      WHEN 'subscription_7' THEN 7
      WHEN 'subscription_30' THEN 30
      ELSE p_days
    END THEN
