...
CHECK ((access_type='free' AND price=0) OR (access_type='paid' AND price BETWEEN 5000 AND 150000)) NOT VALID;
...
CREATE UNIQUE INDEX IF NOT EXISTS cashi_wallet_transactions_user_reference_uidx
ON public.wallet_transactions(user_id, reference)
WHERE user_id IS NOT NULL AND reference IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS cashi_transactions_user_reference_uidx
ON public.transactions(user_id, reference)
WHERE user_id IS NOT NULL AND reference IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS cashi_product_access_order_product_uidx
ON public.product_access(order_id, product_id)
WHERE order_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS cashi_purchases_order_uidx
ON public.purchases(order_id)
WHERE order_id IS NOT NULL;
