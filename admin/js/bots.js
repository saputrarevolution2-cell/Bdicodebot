...
async function load(){
 ...
 try{
  ...
  let data=[];
  try{data=await A.rpc('admin_bots')}catch(e){
    // Read fallback keeps the panel useful when an admin RPC is temporarily unavailable.
    const table='approved_bots';
    const q=await A.sb.from(table).select('*').limit(200);
    if(q.error) throw e; data=A.rows(q.data);
  }
  const toolbar=`<div class="admin-toolbar"><div class="toolbar-title"><i class="fa-solid fa-robot"></i><div><strong>Bots</strong><span>${data.length} data</span></div></div><button class="btn" id="reload"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`;
  box.innerHTML=toolbar+'<div id="adminTable"></div>';
  const cols=[{label:'Id',key:'id'},{label:'Bot Username',key:'bot_username'},{label:'Bot Name',key:'bot_name'},{label:'Bot Id',key:'bot_id'},{label:'Is Active',render:r=>`<span class="status ${ok}">${r.is_active?'Aktif':'Tidak'}</span>`},{label:'Created',render:r=>new Date(r.created_at).toLocaleString('id-ID')}];
