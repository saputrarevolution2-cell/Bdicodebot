...
async function load(){
 ...
 try{
  ...
  let data=[];
  try{data=await A.rpc('admin_products')}catch(e){
    // Read fallback keeps the panel useful when an admin RPC is temporarily unavailable.
    const table='telegram_products';
    const q=await A.sb.from(table).select('*').limit(200);
    if(q.error) throw e; data=A.rows(q.data);
  }
  const toolbar=`<div class="admin-toolbar"><div class="toolbar-title"><i class="fa-solid fa-box"></i><div><strong>Products</strong><span>${data.length} data</span></div></div><button class="btn" id="reload"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`;
  box.innerHTML=toolbar+'<div id="adminTable"></div>';
  const cols=[{label:'Id',key:'id'},{label:'Title',key:'title'},{label:'Slug',key:'slug'},{label:'Price',render:r=>money(r.price)},{label:'Status',key:'status'},{label:'Owner Id',key:'owner_id'},{label:'Created',render:r=>new Date(r.created_at).toLocaleString('id-ID')}];
