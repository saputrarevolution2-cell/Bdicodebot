...
async function load(){
 ...
 try{
  ...
  let data=[];
  try{data=await A.rpc('admin_payments')}catch(e){
  ...
  const toolbar=`<div class="admin-toolbar"><div class="toolbar-title"><i class="fa-solid fa-credit-card"></i><div><strong>Payments</strong><span>${data.length} data</span></div></div><button class="btn" id="reload"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`;
  box.innerHTML=toolbar+'<div id="adminTable"></div>';
  const cols=[{label:'Id',key:'id'},{label:'Order Id',key:'order_id'},{label:'Amount',render:r=>money(r.amount)},{label:'Status',key:'status'},{label:'Provider',key:'provider'},{label:'Created',render:r=>new Date(r.created_at).toLocaleString('id-ID')}];
