...
async function boot(){
 ...
 $('#run')?.addEventListener('click',async()=>{const b=$('#run');b.disabled=true;try{const r=await A.sb.rpc('is_current_user_admin');A.toast(r.error?'Pemeriksaan gagal':(r.data?'Admin terverifikasi':'Bukan admin'),r.error?'error':'ok')}finally{b.disabled=false}});
