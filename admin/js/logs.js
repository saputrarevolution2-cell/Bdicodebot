...
async function load(){
 ...
 try{
  ...
  let data=[];
  try{data=await A.rpc('admin_logs')}catch(e){
  ...
  const toolbar=`<div class="admin-toolbar"><div class="toolbar-title"><i class="fa-solid fa-list-check"></i><div><strong>Logs</strong><span>${data.length} data</span></div></div><button class="btn" id="reload"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`;
  box.innerHTML=toolbar+'<div id="adminTable"></div>';
  const cols=[{label:'Id',key:'id'},{label:'Action',key:'action'},{label:'Admin Id',key:'admin_id'},{label:'Target Type',key:'target_type'},{label:'Target Id',key:'target_id'},{label:'Created',render:r=>new Date(r.created_at).toLocaleString('id-ID')}];
