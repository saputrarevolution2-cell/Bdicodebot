...
async function load(){
 ...
 try{
  ...
  let data=[];
  try{data=await A.rpc('admin_transactions')}catch(e){
  ...
  const toolbar=`<div class="admin-toolbar"><div class="toolbar-title"><i class="fa-solid fa-arrow-right-arrow-left"></i><div><strong>Transactions</strong><span>${data.length} data</span></div></div><button class="btn" id="reload"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`;
  box.innerHTML=toolbar+'<div id="adminTable"></div>';
  const cols=[{label:'Id',key:'id'},{label:'User Id',key:'user_id'},{label:'Type',key:'type'},{label:'Amount',render:r=>money(r.amount)},{label:'Status',key:'status'},{label:'Created',render:r=>new Date(r.created_at).toLocaleString('id-ID')}];
