...
-- Canonical notification broadcaster.
-- IMPORTANT: keep only this 7-argument signature so calls with 3 arguments
-- cannot become ambiguous with an older overloaded function.
DROP FUNCTION IF EXISTS public.notify_all_users(text,text,uuid);
...
BEGIN
  ...
  -- Account/subscription purchase: no seller settlement.
  IF o.item_type='account_plan' THEN
    IF o.item_id='premium' THEN
      UPDATE public.profiles
...
AS $function$
    ...
    WHERE lower(btrim(p.username)) =
          lower(btrim(COALESCE(p_username,'')))
    LIMIT 1;
$function$;

-- ============================================================
-- FINAL EXECUTE PRIVILEGES (after all function definitions)
-- ============================================================
REVOKE ALL ON FUNCTION public.create_pastelink_content(
    text,text,text,text,numeric,text,text[],timestamptz,text
) FROM PUBLIC;
...
-- ============================================================
-- FINAL TRIGGER REBUILD: all referenced functions are defined above
-- ============================================================
BEGIN;
...
-- No procedural audit loop is executed during migration.
-- The canonical functions above explicitly use extensions.digest().
-- ============================================================
...
/* ============================================================
   MARKETPLACE SECURITY/LOAD FIX FROM PROJECT ZIP
   ============================================================ */
...
/* ====================================================================
   PasTele — ADMIN PANEL + ISOLATED LOGIN COMPLETE FINAL PATCH
   2026-09-22

   This patch is intentionally appended to the UNIFIED database above.
   It replaces the lightweight admin RPCs with admin-safe versions that
   match the actual tables created by the canonical schema in this file.

   It includes:
   - isolated admin username login
   - admin access check
   - user list + balances
   - admin password change
   - safe user deletion
   - all marketplace content in one admin Content/Product manager
   - owner username + content body
   - edit title/price/description/status/slug/content
   - order username/search/status/delete
   - payment username + all user balances
   - withdrawal username + approve/reject + notification
   - transaction username + delete
   - bot edit/delete/active
==================================================================== */
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok',false,'reason','PROFILE_NOT_FOUND','id',uid);
  END IF;
...
DROP FUNCTION IF EXISTS public.admin_users(integer,integer);
...
AS $$
  ...
  FROM (
    SELECT
      ...
      p.username,
      p.auth_email,
      p.auth_email AS email,
      p.role,
      coalesce(p.is_admin,false) AS is_admin,
      coalesce(p.is_banned,false) AS is_banned,
      coalesce(w.balance,p.balance,0)::numeric AS balance,
      coalesce(p.balance,0)::numeric AS profile_balance,
      coalesce(w.balance,0)::numeric AS wallet_balance,
      coalesce(w.available_balance,0)::numeric AS available_balance,
      coalesce(w.pending_balance,0)::numeric AS pending_balance,
      p.created_at,
      p.updated_at
    FROM public.profiles p
...
BEGIN
  ...
  IF coalesce(target.is_admin,false)=true
     OR lower(coalesce(target.role,''))='owner' THEN
...
-- ================================================================
-- ADMIN DELETE USER
-- Deleting auth.users cascades to profiles. Content tables that use
-- SET NULL remain; PasteLinks use their canonical CASCADE behavior.
-- Admin/owner and current admin are protected.
-- ================================================================
...
BEGIN
  ...
  IF coalesce(target.is_admin,false)=true
     OR lower(coalesce(target.role,'')) IN ('admin','owner') THEN
...
-- ================================================================
-- COMPLETE CONTENT MANAGER
-- ================================================================

DROP FUNCTION IF EXISTS public.admin_content(integer,integer);
...
AS $$
  ...
  FROM (
    SELECT
      p.id,p.title,p.slug,coalesce(p.price,0)::numeric AS price,p.status,
    ...
    SELECT
      ...
      CASE WHEN lower(coalesce(pl.visibility,'public'))='public' THEN 'published'
           ELSE lower(pl.visibility) END,
      pl.description,coalesce(pl.content_html,'')::text,
      coalesce(pl.views,0)::bigint,0::bigint,
      pl.user_id,pr.username,
    ...
    SELECT
      tp.id,tp.title,tp.slug,coalesce(tp.price,0)::numeric,tp.status,tp.description,
      coalesce(tp.content,'')::text,
      coalesce(tp.views,0)::bigint,coalesce(tp.sales_count,0)::bigint,
      tp.owner_id,pr.username,
    ...
    SELECT
      ...
      tc.description,coalesce(tc.content,'')::text,
      coalesce(tc.views,0)::bigint,coalesce(tc.sales_count,0)::bigint,
      tc.owner_id,pr.username,
      'telegram_channels'::text,
      CASE WHEN lower(coalesce(tc.type,'channel'))='group' THEN 'group'
           ELSE 'channel' END,
    ...
    SELECT
      ...
      CASE WHEN lower(coalesce(ps.visibility,'public'))='public' THEN 'published'
           ELSE lower(ps.visibility) END,
      NULL::text,coalesce(ps.content,'')::text,
      0::bigint,0::bigint,
      ps.owner_id,pr.username,
      'pastes'::text,'paste'::text,'free'::text,
      NULL::text,ps.created_at,ps.updated_at
    FROM public.pastes ps
    LEFT JOIN public.profiles pr ON pr.id=ps.owner_id
    WHERE public.is_current_user_admin()
  ) x
...
DROP FUNCTION IF EXISTS public.admin_update_content(uuid,text,text,text,text,text,numeric);
...
DECLARE
  r jsonb;
  src text:=lower(coalesce(p_source,'products'));
  final_price numeric;
BEGIN
  IF NOT public.is_current_user_admin() THEN
    RAISE EXCEPTION 'ADMIN_REQUIRED';
  ...
  IF p_price IS NOT NULL
     AND (p_price<0 OR p_price>100000) THEN
    RAISE EXCEPTION 'INVALID_PRICE';
  END IF;
  ...
  CASE src
    ...
    WHEN 'pastelinks' THEN
      ...
      SET title=coalesce(nullif(btrim(p_title),''),title),
          ...
          access_type=CASE WHEN coalesce(p_price,price)=0 THEN 'free' ELSE 'paid' END,
          visibility=CASE
            WHEN lower(coalesce(p_status,'')) IN ('published','public','active') THEN 'public'
            WHEN p_status IS NULL THEN visibility
            ELSE p_status END,
          description=coalesce(p_description,description),
          content_html=coalesce(p_content,content_html),
          updated_at=now()
      WHERE id=p_id
    ...
    WHEN 'pastes' THEN
      ...
      SET title=coalesce(nullif(btrim(p_title),''),title),
          ...
          updated_at=now()
      WHERE id=p_id
      RETURNING to_jsonb(pastes.*) INTO r;
...
-- Also allow deleting standalone Paste rows from Content manager.
...
DROP FUNCTION IF EXISTS public.admin_orders(integer,integer);
...
SET search_path=public,extensions
AS $$
DECLARE s text:=lower(btrim(coalesce(p_status,'')));
BEGIN
  IF NOT public.is_current_user_admin() THEN
    RAISE EXCEPTION 'ADMIN_REQUIRED';
  END IF;

  IF s NOT IN ('pending','paid','success','failed','cancelled','completed') THEN
    RAISE EXCEPTION 'INVALID_ORDER_STATUS';
  END IF;

  UPDATE public.orders
  SET status=s,
      paid_at=CASE WHEN s IN ('paid','success','completed') THEN coalesce(paid_at,now()) ELSE paid_at END
  WHERE id=p_order_id;
  ...
  RETURN jsonb_build_object('ok',true,'id',p_order_id,'status',s);
END;
$$;
...
DROP FUNCTION IF EXISTS public.admin_payments(integer,integer);
...
AS $$
  ...
  FROM (
    SELECT
      pay.*,
      coalesce(p.username,pp.username) AS username,
...
-- Existing admin_adjust_balance remains the canonical ledger updater.
GRANT EXECUTE ON FUNCTION public.admin_adjust_balance(uuid,numeric,text) TO authenticated;

-- ================================================================
-- WITHDRAWALS + USER NOTIFICATION
-- ================================================================

DROP FUNCTION IF EXISTS public.admin_withdrawals(integer,integer);
...
DECLARE
  ...
  old_status text;
  new_status text:=lower(btrim(coalesce(p_status,'')));
  refund numeric:=0;
...
BEGIN
  ...
  IF new_status NOT IN ('pending','approved','rejected','completed','cancelled') THEN
    RAISE EXCEPTION 'INVALID_WITHDRAWAL_STATUS';
  END IF;

  IF new_status='rejected' AND note_text='' THEN
  ...
  SET status=new_status,
      note=note_text,
      processed_at=CASE
        WHEN new_status IN ('approved','rejected','completed','cancelled')
        THEN now() ELSE processed_at END
  WHERE id=p_id
  RETURNING * INTO w;

  -- Refund a withdrawal that was previously held, only once.
  IF new_status IN ('rejected','cancelled')
     AND old_status NOT IN ('rejected','cancelled') THEN
    ...
    VALUES(
      w.user_id,refund,0,refund,
      'withdrawal_refund','completed',
      'withdrawal-refund:'||w.id::text,
      'Refund withdrawal rejected/cancelled'
    )
    ON CONFLICT(user_id,reference) DO NOTHING;
  END IF;

  -- Notify the user only when the status actually changes.
  IF old_status IS DISTINCT FROM new_status AND w.user_id IS NOT NULL THEN
    IF new_status='approved' THEN
      nt_title:='Withdrawal Disetujui';
      nt_body:='Withdrawal kamu sebesar Rp '||
        to_char(coalesce(w.net_amount,0),'FM999G999G999G999')||
        ' telah disetujui.';
    ELSIF new_status='completed' THEN
      nt_title:='Withdrawal Berhasil';
      nt_body:='Withdrawal kamu sebesar Rp '||
        to_char(coalesce(w.net_amount,0),'FM999G999G999G999')||
        ' telah selesai.';
    ELSIF new_status='rejected' THEN
      nt_title:='Withdrawal Ditolak';
      nt_body:='Withdrawal kamu ditolak. Alasan: '||note_text;
    ELSIF new_status='cancelled' THEN
      nt_title:='Withdrawal Dibatalkan';
      nt_body:='Withdrawal kamu dibatalkan.'||
        CASE WHEN note_text<>'' THEN ' Alasan: '||note_text ELSE '' END;
    ELSE
      nt_title:='Status Withdrawal Diperbarui';
      nt_body:='Status withdrawal kamu: '||new_status||'.';
    END IF;

    INSERT INTO public.notifications(user_id,title,body,is_read,created_at)
    VALUES(w.user_id,nt_title,nt_body,false,now());
  END IF;
...
DROP FUNCTION IF EXISTS public.admin_transactions(integer,integer);
...
/* ================================================================
   SAFE FINAL CHECKS — these do not use array_agg or procedural
   pg_proc loops and are safe to run after the migration.
================================================================ */
SELECT
  'admin_login_identity' AS function_name,
  to_regprocedure('public.admin_login_identity(text)') IS NOT NULL AS exists;

SELECT
  'admin_set_user_password' AS function_name,
  to_regprocedure('public.admin_set_user_password(uuid,text)') IS NOT NULL AS exists;

SELECT
  'admin_delete_user' AS function_name,
  to_regprocedure('public.admin_delete_user(uuid)') IS NOT NULL AS exists;

SELECT
  'admin_content' AS function_name,
  to_regprocedure('public.admin_content(integer,integer)') IS NOT NULL AS exists;

SELECT
  'admin_update_content' AS function_name,
  to_regprocedure('public.admin_update_content(uuid,text,text,text,text,text,numeric,text)') IS NOT NULL AS exists;

SELECT
  'admin_orders' AS function_name,
  to_regprocedure('public.admin_orders(integer,integer)') IS NOT NULL AS exists;

SELECT
  'admin_payments' AS function_name,
  to_regprocedure('public.admin_payments(integer,integer)') IS NOT NULL AS exists;

SELECT
  'admin_withdrawals' AS function_name,
  to_regprocedure('public.admin_withdrawals(integer,integer)') IS NOT NULL AS exists;

SELECT
  'admin_transactions' AS function_name,
  to_regprocedure('public.admin_transactions(integer,integer)') IS NOT NULL AS exists;
