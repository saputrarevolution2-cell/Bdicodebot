...
(async()=>{const el=document.getElementById('adminContent');try{await Admin.guard();const data=Admin.rows(await Admin.rpc('admin_logs',{p_limit:200,p_offset:0}));el.innerHTML=`<div class="pt-toolbar"><button class="btn" onclick="location.reload()"><i class="fa-solid fa-rotate"></i> Refresh</button></div>`+Admin.table(data)}catch(e){el.innerHTML='<div class="empty">'+Admin.esc(e.message)+'</div>'}})();
/* ============================================================
