...
export async function onRequestPost(context) {
  const {request,env}=context; const body=await readJson(request); const orderId=String(body.order_id||"").trim(); const guestToken=String(body.guest_token||"").trim();
  ...
  try {
    ...
    if(!providerId) return json({success:false,error:"PAYMENT_REFERENCE_NOT_FOUND"},400);
    const r=await fetch(`${env.CASHI_API_URL||"https://cashi.id/api"}/check-status/${encodeURIComponent(providerId)}`,{headers:{"x-api-key":env.CASHI_API_KEY}});
    const data=await r.json().catch(()=>({}));
    if(!r.ok) return json({success:false,error:data?.message||`Cashi HTTP ${r.status}`,raw:data},502);
    const status=String(data?.status||"").toUpperCase();
    if(status==="SETTLED") {
      const settled=await supabaseRpc(env,"settle_cashi_order",{p_order_id:orderId,p_invoice_id:providerId,p_gateway_status:"SETTLED",p_final_amount:Number(order.amount||0),p_gateway_payload:data},"",true);
      return json({success:true,status:"SETTLED",order_id:orderId,orderId:providerId,amount:Number(order.amount||0),settled,raw:data});
    }
    return json({success:true,status:status||"PENDING",order_id:orderId,orderId:providerId,amount:Number(order.amount||0),raw:data});
