...
2. Final compatibility fixes for marketplace, guest access,
   likes/comments, notifications, views and Bayar.gg settlement.
...
/*
====================================================================
FINAL COMPATIBILITY / SAFETY LAYER
====================================================================
This section is intentionally non-destructive.
It does not drop application tables or application data.

The frontend build contains:
- public marketplace/content pages
- account/profile/settings pages
- create Code/PasteLink/Product/Telegram pages
- purchase/payment/wallet/withdrawal pages
- notifications/social interactions
- admin pages
- authentication pages

The canonical RPCs above are the server-side contract for those flows.
The grants below ensure the browser can call only the intended public/
authenticated RPCs. Financial settlement remains service_role-only.
====================================================================
*/

-- Public/authenticated read-only marketplace surface.
GRANT SELECT ON public.marketplace_public TO anon, authenticated;
...
-- Public content view/interaction helpers.
GRANT EXECUTE ON FUNCTION public.record_content_view(uuid,text,uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.toggle_content_like_guest(uuid,text,text) TO anon, authenticated;

-- Authenticated user interactions.
GRANT EXECUTE ON FUNCTION public.toggle_content_like(uuid,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_profile_visit(uuid) TO authenticated;

-- Creator content creation RPCs.
GRANT EXECUTE ON FUNCTION public.create_code_content(text,text,text,text,numeric,text,uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_pastelink_content(text,text,text,text,numeric,text,text[],timestamptz) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_telegram_content(text,text,text,text,numeric,text,text,text,text) TO anon, authenticated;

-- Payment/order creation and account plans.
GRANT EXECUTE ON FUNCTION public.create_checkout_order(text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_account_plan_order(text) TO authenticated;

-- Withdrawal is authenticated-only.
GRANT EXECUTE ON FUNCTION public.request_withdrawal_v2(numeric,text,text,text,text) TO authenticated;

-- Bayar.gg settlement MUST remain server-side only.
REVOKE ALL ON FUNCTION public.settle_bayargg_order(uuid,text,text,numeric,jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.settle_bayargg_order(uuid,text,text,numeric,jsonb) TO service_role;

-- Make sure the guest token can be indexed safely if the column exists.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS orders_guest_access_token_uidx ON public.orders(guest_access_token) WHERE guest_access_token IS NOT NULL';
  END IF;
END $$;

-- Useful performance indexes; all are non-destructive.
CREATE INDEX IF NOT EXISTS idx_products_public_status_created
  ON public.products(status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_telegram_products_public_status_created
  ON public.telegram_products(status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_telegram_channels_public_status_created
  ON public.telegram_channels(status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_pastelinks_public_created
  ON public.pastelinks(created_at DESC);
...
====================================================================
These checks intentionally fail loudly if the deployment is missing
a required application object instead of silently producing a broken
frontend.
====================================================================
*/
DO $$
...
BEGIN
  FOREACH required_table IN ARRAY ARRAY[
    'profiles','products','telegram_products','telegram_channels',
    'pastelinks','pastes','orders','purchases','payments',
    'product_access','product_views','wallets','wallet_transactions',
    'transactions','withdrawals','notifications','content_likes',
    'content_comments','creator_followers','analytics_events'
  ] LOOP
    IF to_regclass('public.' || required_table) IS NULL THEN
      RAISE EXCEPTION 'PasTele database incomplete: missing table public.%', required_table;
    END IF;
  END LOOP;

  IF to_regclass('public.marketplace_public') IS NULL THEN
    RAISE EXCEPTION 'PasTele database incomplete: missing view public.marketplace_public';
  END IF;

  IF to_regclass('public.profile_public') IS NULL THEN
    RAISE EXCEPTION 'PasTele database incomplete: missing view public.profile_public';
  END IF;
END $$;

/*
====================================================================
END — PASTE THIS ONE FILE INTO SUPABASE SQL EDITOR
====================================================================
*/
