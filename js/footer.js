/* PasTele — one shared footer for every public/app/admin page. */
document.addEventListener('DOMContentLoaded', async () => {
  ...
  old.outerHTML = `<footer class="footer pastele-footer">
    ...
    </div><div class="container footer-bottom"><span>© 2026 PasTele. All rights reserved.</span><span>Secure · Responsive · Database driven</span></div></footer>`;
  document.querySelector('[data-footer-logout]')?.addEventListener('click', async () => { try { await Auth.logout(); } catch(e) { TC.toast(e.message,'error'); } });
});
