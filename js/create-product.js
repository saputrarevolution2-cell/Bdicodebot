...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const syncFeatureForm = () => {
    ...
    const map = {
      link: ["fa-link", "Link / URL"],
      paste: ["fa-file-lines", "Paste"],
      pastelink: ["fa-link", "PasteLink"],
      code: ["fa-code", "Code Telegram"],
      channel: ["fa-broadcast-tower", "Channel"],
      group: ["fa-users", "Group"]
    };
    const info = map[type?.value] || map.link;
    const icon = document.querySelector(".create-icon i");
    if (icon) icon.className = "fa-solid " + info[0];
    if (content) {
      content.placeholder = type?.value === "code"
        ? "Tempel kode bot di sini..."
        : (type?.value === "channel" || type?.value === "group"
          ? "https://t.me/username atau @username"
          : "Masukkan content / delivery produk...");
    }
  };
