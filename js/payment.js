/* PasTele — self-contained payment page script. One HTML -> one JS -> one CSS. */
...
document.addEventListener(
    ...
    async () => {
        ...
        if (guestToken && content) {
            ...
            note.innerHTML =
                '<span class="guest-note-icon" aria-hidden="true"><i class="fa-solid fa-user-clock"></i></span>' +
                '<span class="guest-note-copy">' +
                  '<strong>Pembelian Guest berhasil dibuat.</strong>' +
                  '<span>Simpan halaman ini sampai pembayaran selesai. Login/daftar disarankan agar pembelian tersimpan permanen di akun.</span>' +
                '</span>' +
                '<a class="guest-note-login" href="login.html"><i class="fa-solid fa-right-to-bracket"></i><span>Login / Daftar</span></a>';
        ...
        const loadOrder = async () => {
            const { data, error } = await window.PasTeleDB.rpc("get_order_for_payment", {
                p_order_id: orderId,
                        ...
                        setTimeout(
                            () => {
                                window.location.href =
                                    `payment-success.html?order_id=${encodeURIComponent(orderId)}${guestToken ? `&guest_token=${encodeURIComponent(guestToken)}` : ""}`;
                            },
                            600
                        );
                    }
            ...
            async (order) => {
                ...
                try {
                    ...
                    if (
                        ...
                        throw new Error(
                            payment.raw
                                ...
                                payment.raw
                                    ?.message ||
                                "Cashi gagal membuat pembayaran."
                        );
                    }
                    ...
                    if (
                        ...
                        throw new Error(
                            "Cashi tidak mengembalikan QRIS atau payment URL."
                        );
                    }
