/* PasTele — self-contained auth-callback page script. One HTML -> one JS -> one CSS. */
...
/* =========================================================
   PasTele — Google OAuth Account Flow
   Existing account:
     username + Gmail + password + "Masuk sekarang"
   New Google account:
     password + confirmation + "Save" -> automatic login

   Password is NEVER stored in profiles. For a new Google user,
   Supabase Auth receives the password through updateUser().
   The profile itself is created by the existing auth trigger.
   ========================================================= */
(() => {
  'use strict';

  if (window.__PASTELE_GOOGLE_ACCOUNT_FLOW__) return;
  window.__PASTELE_GOOGLE_ACCOUNT_FLOW__ = true;

  const esc = (value) => String(value ?? '').replace(/[&<>"']/g, (m) => ({
    '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#039;'
  }[m]));

  const app = () => document.getElementById('authCallbackApp');
  const setLoading = (message = 'Mohon tunggu.') => {
    const root = app();
    if (!root) return;
    root.innerHTML = `
      <section class="auth-callback-card">
        <div class="auth-callback-loading">
          ...
          <strong>Menyelesaikan login…</strong>
  ...
  const renderError = (message) => {
    ...
    root.innerHTML = `
      <section class="auth-callback-card">
        <div class="auth-callback-head">
          <div class="auth-callback-logo"><i class="fa-solid fa-triangle-exclamation"></i></div>
          <div><h1>Login belum selesai</h1><p>Terjadi masalah saat menyiapkan akun kamu.</p></div>
        </div>
        <div class="auth-callback-body">
          <div class="auth-notice error"><i class="fa-solid fa-circle-exclamation"></i><span>${esc(message || 'Terjadi kesalahan autentikasi.')}</span></div>
          <button class="auth-secondary" id="authBackLogin" type="button"><i class="fa-solid fa-arrow-left"></i> Kembali ke Login</button>
        </div>
      </section>`;
    document.getElementById('authBackLogin')?.addEventListener('click', () => {
  ...
  async function recoverImplicitSession(client) {
    // Current deployed login.js may still return #access_token=... .
    // Convert it into the normal Supabase client session, then remove tokens from the URL.
  ...
  async function getSession(client) {
    ...
    return await recoverImplicitSession(client);
  }

  async function getProfile(client, user) {
    if (!user?.id) return null;
    // The canonical SQL has a handle_new_user trigger, so a new Google user
    // normally gets a profile automatically. Retry briefly for trigger timing.
    for (let i = 0; i < 5; i++) {
      const q = await client.from('profiles')
        .select('id,username,auth_email,display_name,created_at,is_banned')
        .eq('id', user.id)
        .maybeSingle();
  ...
  function userEmail(user, profile) {
    return String(profile?.auth_email || user?.email || '').trim().toLowerCase();
  }

  function looksNewGoogleAccount(user, profile) {
    if (!profile || !user?.created_at) return !profile;
    const created = Date.parse(user.created_at);
    const profileCreated = Date.parse(profile.created_at || '');
    if (!Number.isFinite(created) || !Number.isFinite(profileCreated)) return false;
    // A profile generated by handle_new_user is created immediately after auth.users.
    return Math.abs(profileCreated - created) <= 2 * 60 * 1000;
  }

  function renderExisting({ user, profile, email }) {
    const root = app();
    const username = profile?.username || profile?.display_name || user?.user_metadata?.user_name || 'User';
    root.innerHTML = `
      <section class="auth-callback-card">
        <div class="auth-callback-head">
          <div class="auth-callback-logo"><i class="fa-brands fa-google"></i></div>
          <div>
            <h1>Akun ditemukan</h1>
            <p>Gmail ini sudah terdaftar. Masukkan kata sandi akun PasTele untuk melanjutkan.</p>
          </div>
        </div>
        <div class="auth-callback-body">
          <div class="auth-account-box">
            <div class="auth-account-row"><i class="fa-solid fa-user"></i><div><small>Username</small><strong>${esc(username)}</strong></div></div>
            <div class="auth-account-row"><i class="fa-solid fa-envelope"></i><div><small>Gmail</small><strong>${esc(email)}</strong></div></div>
          </div>
          <div class="auth-notice"><i class="fa-solid fa-shield-halved"></i><span>Gmail sudah terhubung dengan akun terdaftar. Kata sandi hanya dipakai untuk login dan tidak disimpan di halaman ini.</span></div>
          <form id="existingLoginForm" novalidate>
            <div class="auth-field"><label for="existingPassword">Kata sandi</label><input id="existingPassword" name="password" type="password" minlength="6" autocomplete="current-password" placeholder="Masukkan kata sandi" required></div>
            <div id="existingError" class="auth-notice error" style="display:none;margin-top:13px;margin-bottom:0"></div>
            <button class="auth-submit" id="existingLoginButton" type="submit"><i class="fa-solid fa-right-to-bracket"></i> Masuk sekarang</button>
          </form>
          <button class="auth-secondary" id="existingBack" type="button">Kembali ke Login</button>
        </div>
        <div class="auth-callback-footer">Akun kamu tetap menggunakan Gmail dan kata sandi Supabase Auth.</div>
      </section>`;

    const form = document.getElementById('existingLoginForm');
    const btn = document.getElementById('existingLoginButton');
    const errorBox = document.getElementById('existingError');
    form?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const password = String(new FormData(form).get('password') || '');
      if (password.length < 6) {
        errorBox.style.display = 'flex';
        errorBox.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i><span>Kata sandi minimal 6 karakter.</span>';
        return;
      }
      btn.disabled = true;
      ...
      try {
        ...
        if (data.user.id !== user.id) throw new Error('Gmail tidak cocok dengan akun yang dipilih.');
        window.location.replace('/dashboard.html');
      } catch (error) {
        errorBox.style.display = 'flex';
        errorBox.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i><span>' + esc(window.Auth?.getErrorMessage?.(error) || error?.message || 'Username/Gmail atau kata sandi salah.') + '</span>';
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-right-to-bracket"></i> Masuk sekarang';
      }
    });
    document.getElementById('existingBack')?.addEventListener('click', () => window.location.replace('/login.html'));
  }

  function renderNew({ user, email, profile }) {
    const root = app();
    const username = profile?.username || user?.user_metadata?.user_name || user?.user_metadata?.preferred_username || email.split('@')[0];
    root.innerHTML = `
      <section class="auth-callback-card">
        <div class="auth-callback-head">
          <div class="auth-callback-logo"><i class="fa-brands fa-google"></i></div>
          <div>
            <h1>Buat kata sandi akun</h1>
            <p>Gmail kamu belum terdaftar. Buat kata sandi untuk menyelesaikan pendaftaran.</p>
          </div>
        </div>
        <div class="auth-callback-body">
          <div class="auth-account-box">
          ...
          <div class="auth-notice success"><i class="fa-solid fa-circle-check"></i><span>Gmail berhasil diverifikasi. Masukkan kata sandi dan konfirmasi kata sandi, lalu tekan Save. Setelah disimpan kamu otomatis masuk ke dashboard.</span></div>
          <form id="newAccountForm" novalidate>
            <div class="auth-field"><label for="newPassword">Kata sandi</label><input id="newPassword" name="password" type="password" minlength="6" autocomplete="new-password" placeholder="Minimal 6 karakter" required></div>
            <div class="auth-field"><label for="confirmPassword">Konfirmasi kata sandi</label><input id="confirmPassword" name="confirm" type="password" minlength="6" autocomplete="new-password" placeholder="Ulangi kata sandi" required></div>
            <div id="newError" class="auth-notice error" style="display:none;margin-top:13px;margin-bottom:0"></div>
            <button class="auth-submit" id="saveAccountButton" type="submit"><i class="fa-solid fa-floppy-disk"></i> Save</button>
          </form>
        </div>
        <div class="auth-callback-footer">Kata sandi diproses oleh Supabase Auth, bukan disimpan di profile.</div>
      </section>`;

    const form = document.getElementById('newAccountForm');
    const btn = document.getElementById('saveAccountButton');
    const errorBox = document.getElementById('newError');
    form?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const fd = new FormData(form);
      const password = String(fd.get('password') || '');
      const confirm = String(fd.get('confirm') || '');
      if (password.length < 6) return showError(errorBox, 'Kata sandi minimal 6 karakter.');
      if (password !== confirm) return showError(errorBox, 'Konfirmasi kata sandi tidak sama.');
      btn.disabled = true;
      ...
      try {
        ...
        if (error) throw error;
        if (!data?.user) throw new Error('Kata sandi gagal disimpan.');
        // Profile creation is handled by the existing handle_new_user trigger.
        window.location.replace('/dashboard.html');
      } catch (error) {
        showError(errorBox, error?.message || 'Kata sandi gagal disimpan. Silakan coba lagi.');
        btn.disabled = false;
  ...
  function showError(box, message) {
    ...
    box.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i><span>' + esc(message) + '</span>';
  }

  async function boot() {
    if (document.body) {
      document.body.classList.add('google-account-callback');
      document.querySelectorAll('#navbar,.pastele-footer').forEach(el => { el.style.display = 'none'; });
    }
    setLoading('Memeriksa akun Google kamu…');
    try {
      const client = getClient();
      if (!client) throw new Error('Supabase belum siap. Pastikan konfigurasi Supabase tersedia.');
      let session = await getSession(client);
      if (!session) throw new Error('Session Google tidak ditemukan. Silakan ulangi login dengan Google.');

      // Never leave OAuth tokens visible in the browser address bar.
      cleanOAuthUrl();
      ...
      if (profile?.is_banned === true) {
        await client.auth.signOut();
        throw new Error('Akun kamu telah diblokir.');
      }

      if (isNew) {
        renderNew({ user, email, profile });
      } else {
        if (!profile) throw new Error('Profile akun belum tersedia. Silakan tunggu beberapa detik lalu coba login Google lagi.');
        renderExisting({ user, profile, email });
      }
    } catch (error) {
      console.error('[PasTele Google Account Flow]', error);
      renderError(error?.message || 'Terjadi kesalahan autentikasi.');
    }
  }
