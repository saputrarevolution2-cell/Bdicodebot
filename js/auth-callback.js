...
(() => {
  ...
  function looksNewGoogleAccount(user, profile) {
    // If the account already has an email identity, it is an existing
    // PasTele account. Google OAuth has already authenticated the user,
    // so NEVER ask for the PasTele password again.
    const providers = Array.isArray(user?.app_metadata?.providers)
      ? user.app_metadata.providers.map(v => String(v).toLowerCase())
      : [];
    if (providers.includes('email')) return false;

    // A Google-only account with no profile is definitely new.
    if (!profile) return true;

    // For a Google-only account, a profile created immediately after the
    // auth user is the normal new-account trigger flow.
    ...
    root.innerHTML = `
      <section class="auth-callback-card">
        <div class="auth-callback-loading">
          ...
          <strong>Akun berhasil diverifikasi</strong>
          <span>${esc(email)}${username ? ' · @' + esc(username) : ''}</span>
          <small style="color:var(--muted)">Mengalihkan ke dashboard…</small>
        </div>
      </section>`;
    window.setTimeout(() => window.location.replace('/dashboard.html'), 150);
  }
