...
/* Manual WD hard lock */
(function(){const apply=()=>{const box=document.getElementById('manualBox'),st=document.getElementById('manualStatus');if(!box||!st)return;const closed=st.dataset.state==='closed'||st.classList.contains('closed')||document.getElementById('manualSubmit')?.disabled;box.classList.toggle('withdraw-manual-closed',!!closed);box.querySelectorAll('input,select,textarea,button').forEach(el=>{if(closed){el.setAttribute('aria-disabled','true');el.tabIndex=-1;if('disabled' in el)el.disabled=true}})};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply);else apply();new MutationObserver(apply).observe(document.body,{subtree:true,attributes:true,attributeFilter:['class','data-state','disabled']})})();
