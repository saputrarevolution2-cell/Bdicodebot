/* PasTele — self-contained create-pastelink page script. One HTML -> one JS -> one CSS. */
...
window.PasTelePasswordToggle = function(enabled){
  ...
  if(pw){
    pw.disabled=!on;
    pw.required=false;
    if(!on){pw.value="";pw.type="password";}
  }
  if(on && pw) setTimeout(()=>{try{pw.focus({preventScroll:true})}catch(_){pw.focus()}},20);
};
...
function validate(){if(passwordEnabled()&&getPassword().trim()!==""&&getPassword().trim().length<4)return TC.toast("Jika diisi, password minimal 4 karakter.","error"),null;
...
$("createForm")?.addEventListener("submit",async e=>{e.preventDefault();if($("submitBtn").disabled)return;const v=validate();if(!v)return;setLoading(true);try{requireClient();if(v.a==="paid"&&!(await window.TC?.user?.())){toast("Konten Paid hanya bisa dibuat setelah login atau daftar akun.","error");const next=`${location.pathname}${location.search}${location.hash}`;const loginUrl=`login.html?next=${encodeURIComponent(next)}&reason=paid-create`;setLoading(false);window.setTimeout(()=>window.location.assign(loginUrl),450);return;}const slug=await createUniqueShortCode(sb());
const rpcResult=await sb().rpc("create_pastelink_content",{p_title:v.title,p_content:v.content,p_slug:slug,p_access_type:v.a,p_price:v.price,p_description:v.desc,p_tags:v.tags,p_expires_at:v.exp,p_password:(passwordEnabled() && getPassword().trim() ? getPassword().trim() : null)});
const data=rpcResult?.data;
const error=rpcResult?.error;
if(error) throw error;
const payload=Array.isArray(data)?data[0]:data;
if(!payload?.ok) throw new Error("Database tidak mengonfirmasi pembuatan PasteLink.");
$("createForm").reset();
setupPasswordControl();
sync();
$("expiryBox").hidden=true;
$("counter").textContent="0";
finish(v,payload.slug||slug);
toast("PasteLink berhasil dipublikasikan.","success")}catch(e){
  console.error("[PasteLink] create_pastelink_content failed:",e);
  const code=String(e?.code||"");
  const msg=String(e?.message||e||"").trim();
  const friendly =
    code==="23505" ? "Custom URL sudah digunakan. Silakan coba lagi." :
    /TITLE_AND_CONTENT_REQUIRED/i.test(msg) ? "Judul dan isi konten wajib diisi." :
    /INVALID_ACCESS_TYPE/i.test(msg) ? "Jenis akses tidak valid." :
    /LOGIN_REQUIRED_FOR_PAID/i.test(msg) ? "PasteLink Paid hanya bisa dibuat setelah login." :
    /INVALID_PAID_PRICE/i.test(msg) ? "Harga Paid harus Rp2.000–Rp100.000 dan kelipatan Rp1.000." :
    /PASSWORD_TOO_LONG/i.test(msg) ? "Password terlalu panjang." :
    msg || "Gagal membuat PasteLink.";
  toast(friendly,"error");
}finally{setLoading(false)}});sync();$("expiryBox").hidden=true;
});
