...
/* =========================================================
   ...
   ========================================================= */
/* =========================================================
   PasTele — DASHBOARD REDESIGN
   Database-first dashboard UI
   ---------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const supabase = window.sb || window.supabaseClient || window.supabase;

  const esc = value => typeof core.esc === 'function' ? core.esc(value) : String(value ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
  const number = value => Number(value || 0).toLocaleString('id-ID');
  const money = value => typeof core.money === 'function' ? core.money(Number(value || 0)) : new Intl.NumberFormat('id-ID',{style:'currency',currency:'IDR',maximumFractionDigits:0}).format(Number(value || 0));
  const toast = (m,t='info') => { try { core.toast?.(m,t); } catch {} };
  ...
  const formatDate = value => { const d=safeDate(value); return d?d.toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'}):'-'; };
  const formatShort = value => { const d=safeDate(value); return d?d.toLocaleDateString('id-ID',{day:'2-digit',month:'short'}):'-'; };
  ...
  let user=null;
  try{
    user = typeof core.user==='function' ? await core.user() : (await supabase.auth.getUser()).data?.user || null;
  }catch(e){ user=null; }
  ...
  const contentMatches = {
    product(item,s){
      ...
      if(s==='paste') return ['link','paste','pastelink'].includes(t);
      if(s==='code') return t==='code';
      return false;
    },
    telegram(item,s){
      if(s==='all') return true;
      const t=normalize(item?.product_type || item?.type);
      return s==='code' && ['code','product','file'].includes(t);
    },
    channel(item,s){
      if(s==='all') return true;
      if(s==='channel') return ['channel','group'].includes(normalize(item?.type));
      return false;
    },
    event(item,s){
      if(s==='all') return true;
      const t=normalize(item?.target_type);
      if(s==='paste') return ['link','paste','pastelink'].includes(t);
      if(s==='code') return ['code','product','file'].includes(t);
      if(s==='channel') return ['channel','telegram_channel','group','telegram_group'].includes(t);
      return false;
    }
  };

  const periodDates = () => {
    const currentStart=addDays(now,-(period-1));
    const previousStart=addDays(now,-(period*2-1));
    const current=Array.from({length:period},(_,i)=>addDays(currentStart,i));
    const previous=Array.from({length:period},(_,i)=>addDays(previousStart,i));
    return {currentStart,previousStart,current,previous,queryStart:previousStart.toISOString(),queryEnd:addDays(now,1).toISOString()};
  };

  async function load(){
    const pd=periodDates();
    try { await supabase.rpc('release_matured_wallet'); } catch {}
    ...
    const [products,pastes,pastelinks,telegramProducts,telegramChannels,orders,transactions,analyticsEvents,likesResult,followsResult,followerRowsResult,walletResult] = await Promise.all([
      ...
      supabase.from('creator_followers').select('follower_id').eq('creator_id',user.id),
    ...
    const scopedPastelinks=(scope==='all'||scope==='paste')?pastelinks:[];
    const scopedCodes=telegramProducts.filter(x=>contentMatches.telegram(x,scope));
    const scopedChannels=telegramChannels.filter(x=>contentMatches.channel(x,scope));

    const allContent=[
      ...products.map(x=>({...x,kind:normalize(x.type)==='code'?'code':'paste'})),
      ...pastes.map(x=>({...x,kind:'paste'})),
      ...pastelinks.map(x=>({...x,kind:'paste'})),
      ...telegramProducts.map(x=>({...x,kind:'code'})),
      ...telegramChannels.map(x=>({...x,kind:normalize(x.type)==='group'?'group':'channel'}))
    ];
    const filteredContent=[
      ...scopedProducts.map(x=>({...x,kind:normalize(x.type)==='code'?'code':'paste'})),
      ...scopedPastes.map(x=>({...x,kind:'paste'})),
    ...
    const availableNet=Number(wallet.available_balance||0);
    $('heroBalance') && ($('heroBalance').textContent=money(availableNet));
    $('availableHero') && ($('availableHero').textContent=money(availableNet));

    // Total content and status.
    const monthStart=new Date(now.getFullYear(),now.getMonth(),1);
    const monthContent=allContent.filter(x=>safeDate(x.created_at)>=monthStart).length;
    $('monthContent') && ($('monthContent').textContent=number(monthContent));
    $('followerCount') && ($('followerCount').textContent=number(followerCount));
    $('likeCount') && ($('likeCount').textContent=number(likeCount));
    $('shareCount') && ($('shareCount').textContent=number(shareCount));

    // Finance overview values remain database-backed.
    // IMPORTANT: Dashboard revenue is NET. transactions.net_amount is the
    // post-fee seller earning. Never subtract fee again in the UI.
    const saleTransactions=transactions.filter(t=>normalize(t.type)==='sale_earning');
    const available=Number(wallet.available_balance||0);
    const pending=Number(wallet.pending_balance||0);
    const todayKey=dateKey(now);
    const monthKey=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
    const todayRevenue=saleTransactions.filter(t=>dateKey(t.created_at)===todayKey).reduce((s,t)=>s+Number(t.net_amount ?? 0),0);
    const monthRevenue=saleTransactions.filter(t=>{const d=safeDate(t.created_at);return d&&dateKey(d).startsWith(monthKey)}).reduce((s,t)=>s+Number(t.net_amount ?? 0),0);
    $('financePending') && ($('financePending').textContent=money(pending));
    $('financeAvailable') && ($('financeAvailable').textContent=money(available));
    $('financeToday') && ($('financeToday').textContent=money(todayRevenue));
    $('financeMonth') && ($('financeMonth').textContent=money(monthRevenue));

    // Main finance/analytics series: seller earnings + successful orders.
    const successSet=new Set(['paid','success','completed','settled']);
    const revenueForDay=(d,scopeMode='all')=>{
      const k=dateKey(d);
      return saleTransactions.filter(t=>dateKey(t.created_at)===k).filter(t=>scopeMode==='all'||contentMatches.event({target_type:t.reference||''},scopeMode)).reduce((s,t)=>s+Number(t.net_amount ?? 0),0);
    };
    const ordersForDay=(d)=>orders.filter(o=>dateKey(o.paid_at||o.created_at)===dateKey(d)&&successSet.has(normalize(o.status))).length;
    const series=(days)=>days.map(d=>({date:new Date(d),key:dateKey(d),revenue:revenueForDay(d),transactions:ordersForDay(d)}));
    ...
    const ids=followerRows.map(x=>x.follower_id).filter(Boolean);
    let followerProfiles=[];
    if(ids.length){
      try { followerProfiles=(await supabase.from('profiles').select('id,country').in('id',ids)).data||[]; } catch {}
    }
  ...
  function svgPath(values, width, height, max, padX=14, padY=16){
    if(!values.length) return '';
    const innerW=Math.max(1,width-padX*2), innerH=Math.max(1,height-padY*2);
    return values.map((v,i)=>{
      const x=padX+(values.length===1?innerW/2:(i/(values.length-1))*innerW);
      const y=height-padY-(Number(v||0)/Math.max(1,max))*innerH;
  ...
  function renderFollowers(total,profiles){
    const meta={
      indonesia:{label:'Indonesia',flag:'🇮🇩',color:'#ef4444'},
      malaysia:{label:'Malaysia',flag:'🇲🇾',color:'#facc15'},
      china:{label:'China',flag:'🇨🇳',color:'#991b1b'},
      singapore:{label:'Singapore',flag:'🇸🇬',color:'#ef4444'},
      japan:{label:'Japan',flag:'🇯🇵',color:'#f8fafc'},
      thailand:{label:'Thailand',flag:'🇹🇭',color:'#ef4444'},
      vietnam:{label:'Vietnam',flag:'🇻🇳',color:'#eab308'},
      philippines:{label:'Philippines',flag:'🇵🇭',color:'#38bdf8'},
      india:{label:'India',flag:'🇮🇳',color:'#f97316'},
      usa:{label:'United States',flag:'🇺🇸',color:'#2563eb'},
      'united states':{label:'United States',flag:'🇺🇸',color:'#2563eb'},
      uk:{label:'United Kingdom',flag:'🇬🇧',color:'#6366f1'},
      'united kingdom':{label:'United Kingdom',flag:'🇬🇧',color:'#6366f1'}
    };
    const counts={};
    profiles.forEach(p=>{const k=normalize(p.country)||'unknown';counts[k]=(counts[k]||0)+1;});
    const unknown=Math.max(0,total-profiles.length);if(unknown)counts.unknown=unknown;
    const items=Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([key,count])=>({key,count,...(meta[key]||{label:key==='unknown'?'Negara tidak diketahui':key.replace(/\b\w/g,m=>m.toUpperCase()),flag:'🌐',color:'#94a3b8'})}));
    let cursor=0;const stops=items.map(x=>{const p=total?x.count/total*100:0;const f=cursor;cursor+=p;return `${x.color} ${f}% ${cursor}%`;}).join(',');
    $('followerDonut') && ($('followerDonut').style.background=`conic-gradient(${stops||'#273244 0 100%'})`);
    $('followerTotal') && ($('followerTotal').textContent=number(total));
    const host=$('followerCountries');if(!host)return;
    host.innerHTML=items.length?items.slice(0,10).map(x=>{const p=total?x.count/total*100:0;return `<button type="button" class="country-pill" data-country="${esc(x.key)}" style="--pill-color:${x.color}"><span>${x.flag}</span><b>${esc(x.label)}</b><em>${number(x.count)} · ${p.toFixed(p>=10?0:1)}%</em></button>`;}).join(''):`<div class="empty"><i class="fa-solid fa-users"></i><span>Belum ada pengikut.</span></div>`;
    host.querySelectorAll('.country-pill').forEach(btn=>btn.addEventListener('click',()=>{host.querySelectorAll('.country-pill').forEach(x=>x.classList.remove('active'));btn.classList.add('active');}));
  }
  ...
  try{await load();}catch(e){console.error('Dashboard load error',e);$('dashboardError')?.classList.remove('hidden');toast(e?.message||'Dashboard gagal dimuat.','error');}
});
