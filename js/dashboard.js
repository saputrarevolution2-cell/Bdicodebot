...
function withTimeout(promise,ms=12000,label='Request timeout'){
 return Promise.race([promise,new Promise((_,reject)=>setTimeout(()=>reject(new Error(label)),ms))]);
}
function dashboardLoading(text='Memuat dashboard...'){
 const c=$('#content');if(c)c.innerHTML=`<section class="card" style="padding:32px;text-align:center"><div style="font-size:28px;margin-bottom:10px"><i class="fa-solid fa-spinner fa-spin"></i></div><strong>${escape(text)}</strong><p class="muted">Mohon tunggu...</p></section>`;
}
function initials(s){return (s||'U').replace('@','').slice(0,1).toUpperCase()}
function setTheme(){const t=localStorage.getItem('telecod_theme')==='light'?'light':'dark';document.documentElement.dataset.theme=t;document.documentElement.classList.toggle('light',t==='light');document.documentElement.classList.toggle('dark',t!=='light');document.body.classList.toggle('light',t==='light');document.body.classList.toggle('dark',t!=='light');}
function applyLang(){
  document.documentElement.lang=state.lang;
  $$('[data-i18n]').forEach(x=>x.textContent=tr(x.dataset.i18n));
  if($('#langBtn'))$('#langBtn').textContent=state.lang.toUpperCase();
  window.TeleCodUI?.setLang?.(state.lang);
...
function layoutReady(){
 ...
 $('#logoutBtn').onclick=async e=>{e.preventDefault();if(sup)await sup.auth.signOut();location.href='/index.html'};$('#userBtn').onclick=()=>location.href='/dashboard/profile.html';setTheme();applyLang();
}
async function loadUser(){
 dashboardLoading('Memeriksa sesi login...');
 if(!sup){toast(tr('config'),'warning');return true}
 try{
  const sr=await withTimeout(sup.auth.getSession(),10000,'Session check timeout.');
  if(sr?.error||!sr?.data?.session){location.replace('/auth/login.html');return false}
  state.user=sr.data.session.user;
  try{
   const pr=await withTimeout(sup.from('profiles').select('*').eq('id',state.user.id).maybeSingle(),10000,'Profile request timeout.');
   state.profile=pr?.data||{};
  }catch(e){console.warn('Profile load skipped:',e?.message);state.profile={}}
  if(state.profile.is_banned){try{await withTimeout(sup.auth.signOut(),3000,'Signout timeout')}catch(e){}location.replace('/index.html?banned=1');return false}
  if(state.profile.is_admin || String(state.profile.telegram_id||"").replace(/\D/g,"")===MASTER_ADMIN_ID){const admin=$('#adminNav');if(admin)admin.style.display='flex'}
  try{await withTimeout(sup.rpc('ensure_wallet',{p_user:state.user.id}),8000,'Wallet initialization timeout.')}catch(e){console.warn('ensure_wallet skipped:',e?.message)}
  const username=state.profile.username||state.user.user_metadata?.username||'user';
  if($('#topUser'))$('#topUser').textContent='@'+username;
  if($('#avatar'))$('#avatar').textContent=initials(state.profile.username||state.user.email);
  return true;
 }catch(err){
  console.error('loadUser:',err);
  const c=$('#content');
  if(c)c.innerHTML=`<section class="card" style="padding:32px;text-align:center"><div style="font-size:28px">⚠️</div><h2>Gagal memuat dashboard</h2><p class="muted">${escape(err?.message||'Koneksi atau session bermasalah.')}</p><div class="actions" style="justify-content:center"><button class="btn btn-primary" id="retryDashboard">Coba Lagi</button><button class="btn btn-secondary" id="loginAgain">Login Ulang</button></div></section>`;
  $('#retryDashboard')?.addEventListener('click',()=>location.reload());
  $('#loginAgain')?.addEventListener('click',()=>location.replace('/auth/login.html'));
  return false;
 }
}
...
async function renderCreate(type,editId=null){
      ...
      <form id="productForm" class="creator-form">
        ...
        <div class="creator-form-main">
          ...
          <section class="create-section">
            <div class="create-section-head">
              ...
              <div>
                <h2>${tr('productInfo')}</h2>
                <p>${tr('productInfoHint')}</p>
              </div>
            </div>

            <div class="form-grid">
              <div class="form-group">
                <label class="form-label" for="fTitle">${tr('productName')} <span>*</span></label>
                <div class="input-wrap">
                  <i class="fa-solid ${isChannel?'fa-bullhorn':'fa-tag'}"></i>
                  <input class="input" id="fTitle" value="${escape(p?.title||'')}" placeholder="${isChannel?tr('channelNamePlaceholder'):tr('codeNamePlaceholder')}" required>
                </div>
              </div>

              <div class="form-group">
                <label class="form-label" for="fCategory">${tr('category')}</label>
                <div class="input-wrap">
                  <i class="fa-solid fa-folder"></i>
                  <input class="input" id="fCategory" value="${escape(p?.category||'')}" placeholder="${tr('categoryPlaceholder')}">
                </div>
              </div>

              <div class="form-group full">
                <label class="form-label" for="fDesc">${tr('description')}</label>
                ...
                <small class="field-help">${tr('description')} yang jelas membuat pembeli lebih mudah memahami produk.</small>
              </div>
            </div>
          </section>
          ...
          <section class="create-section">
            ...
            <div class="price-field" id="priceField">
              <label class="form-label" for="fPrice">${tr('price')} Produk <span>*</span></label>
          ...
          <section class="create-section">
            ...
            <div class="form-group">
              <label class="form-label" for="fTelegram">${tr('channelLink')} <span>*</span></label>
        ...
        <aside class="creator-form-side">
          <div class="create-preview-card">
            ...
            <p id="previewDesc">${escape(p?.description||tr('description')+' produk akan tampil di sini.')}</p>
  ...
  $('#fDesc').oninput=()=>$('#previewDesc').textContent=$('#fDesc').value.trim()||tr('description')+' produk akan tampil di sini.';
  syncAccess();
...
async function render(){const path=document.body.dataset.dashboardPage||'';const page=(path==='index'?'dashboard':path)||new URLSearchParams(location.search).get('page')||'dashboard';closeMobile();if(page==='dashboard')return renderDashboard();if(page==='pastelink'){location.href='/index.html#paste';return;}if(page==='marketplace')return renderMarketplace();if(page==='purchases')return renderPurchases();if(page==='payment')return renderPayment();if(page==='profile')return renderProfile();if(page==='settings')return renderSettings();if(page==='create-channel')return renderCreate('channel');if(page==='create-code')return renderCreate('code');if(['channel-free','channel-paid','code-free','code-paid'].includes(page))return renderProducts(page);return renderDashboard()}
async function bootDashboard(){
 try{layoutReady();dashboardLoading('Memuat akun...');const ok=await loadUser();if(ok)await render();}
 catch(err){console.error('Dashboard boot error:',err);const c=$('#content');if(c)c.innerHTML='<section class="card" style="padding:32px;text-align:center"><h2>Dashboard gagal dimuat</h2><p class="muted">Terjadi kesalahan saat memulai halaman.</p><button class="btn btn-primary" onclick="location.reload()">Coba Lagi</button></section>'}
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bootDashboard,{once:true});else bootDashboard();
})();
