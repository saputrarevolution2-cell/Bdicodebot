...
function setTheme(){document.documentElement.dataset.theme=state.theme;localStorage.setItem('telecod_theme',state.theme)}
...
function layoutReady(){
 ...
 $('#themeBtn').onclick=()=>{state.theme=state.theme==='dark'?'light':'dark';setTheme()};$('#langBtn').onclick=()=>{state.lang=state.lang==='id'?'en':'id';localStorage.setItem('telecod_lang',state.lang);applyLang();render()};
 $$('.nav-parent').forEach(b=>b.onclick=()=>$('#submenu-'+b.dataset.toggle).classList.toggle('open'));
 $('#logoutBtn').onclick=async e=>{e.preventDefault();if(sup)await sup.auth.signOut();location.href='index.html'};$('#userBtn').onclick=()=>location.href='dashboard.html?page=profile';$('#notifBtn').onclick=()=>toast(state.lang==='id'?'Tidak ada notifikasi baru.':'No new notifications.');setTheme();applyLang();
}
...
async function renderDashboard(){setActive('dashboard');const [{data:products},{data:purchases},{data:tx},{data:w}]=await Promise.all([sup.from('products').select('id,views').eq('creator_id',state.user.id),sup.from('purchases').select('id,amount,status').eq('buyer_id',state.user.id),sup.from('transactions').select('*').eq('user_id',state.user.id).order('created_at',{ascending:false}).limit(6),sup.from('wallets').select('balance').eq('user_id',state.user.id).maybeSingle()]);const views=(products||[]).reduce((a,x)=>a+Number(x.views||0),0),spent=(purchases||[]).filter(x=>x.status==='paid').reduce((a,x)=>a+Number(x.amount),0),earned=(tx||[]).filter(x=>x.type==='sale'&&x.status==='success').reduce((a,x)=>a+Number(x.amount),0);$('#content').innerHTML=`<div class="page-head"><div><div class="eyebrow">TELECOD</div><h1 class="page-title">${tr('welcome')}, @${escape(state.profile.username||'user')} 👋</h1><p class="page-sub">${tr('overview')}</p></div><a class="btn btn-primary" href="dashboard.html?page=create-channel"><i class="fa-solid fa-plus"></i> ${tr('createProduct')}</a></div><div class="grid stats-grid"><div class="card"><div class="stat-label">${tr('products')}</div><div class="stat-value">${products?.length||0}</div></div><div class="card"><div class="stat-label">${tr('views')}</div><div class="stat-value">${views.toLocaleString()}</div></div><div class="card"><div class="stat-label">${tr('earned')}</div><div class="stat-value">${money(earned)}</div></div><div class="card"><div class="stat-label">${tr('balance')}</div><div class="stat-value">${money(w?.balance||0)}</div></div></div><div class="grid two" style="margin-top:15px"><section class="card"><h2 class="section-title"><i class="fa-solid fa-clock-rotate-left"></i> ${tr('recent')}</h2>${tx?.length?`<div class="table-wrap"><table class="table"><thead><tr><th>${tr('date')}</th><th>${tr('description')}</th><th>${tr('amount')}</th><th>${tr('status')}</th></tr></thead><tbody>${tx.map(x=>`<tr><td>${new Date(x.created_at).toLocaleDateString()}</td><td>${escape(x.description||x.type)}</td><td class="${x.direction==='credit'?'money-plus':'money-minus'}">${x.direction==='credit'?'+':'-'}${money(x.amount)}</td><td>${escape(x.status)}</td></tr>`).join('')}</tbody></table></div>`:`<div class="empty"><i class="fa-solid fa-chart-line big"></i><br>${tr('empty')}</div>`}</section><section class="card"><h2 class="section-title"><i class="fa-solid fa-bolt"></i> ${tr('quick')}</h2><div class="grid"><a class="btn btn-primary" href="dashboard.html?page=create-channel"><i class="fa-solid fa-bullhorn"></i> ${tr('createChannel')}</a><a class="btn btn-secondary" href="dashboard.html?page=create-code"><i class="fa-solid fa-code"></i> ${tr('createCode')}</a><a class="btn btn-secondary" href="dashboard.html?page=marketplace"><i class="fa-solid fa-cart-shopping"></i> ${tr('marketplace')}</a></div></section></div>`}
...
async function renderPayment(){
  setActive('payment');
  try{ await sup.rpc('release_matured_sales'); }catch(_){}
  const [{data:w},{data:tx},{data:wd}]=await Promise.all([
    ...
    sup.from('transactions').select('*').eq('user_id',state.user.id).order('created_at',{ascending:false}).limit(50),
    sup.from('withdrawals').select('*').eq('user_id',state.user.id).order('created_at',{ascending:false}).limit(30)
  ]);
  const available=Number(w?.balance||0),pending=Number(w?.pending_balance||0);
  $('#content').innerHTML=`
    <div class="page-head"><div><div class="eyebrow">ACCOUNT</div><h1 class="page-title">${tr('paymentTitle')}</h1>
    <p class="page-sub">Saldo penjualan masuk ke Pending dan otomatis menjadi Available setelah H+1.</p></div></div>
    <div class="grid three">
      <div class="card"><div class="stat-label">Saldo Tersedia</div><div class="balance">${money(available)}</div></div>
      <div class="card"><div class="stat-label">Saldo Pending · H+1</div><div class="balance">${money(pending)}</div><div class="help">Penjualan baru tersedia setelah 1 hari.</div></div>
      <div class="card"><div class="stat-label"><i class="fa-solid fa-circle-plus"></i> ${tr('deposit')}</div>
        <button class="btn btn-primary" style="margin-top:10px" id="depositBtn"><i class="fa-solid fa-circle-plus"></i> ${tr('deposit')}</button>
        <div class="help">${tr('depositHint')}</div>
      </div>
    </div>
    <div class="card" style="margin-top:15px">
      <div class="panel-head"><h2 class="section-title"><i class="fa-solid fa-money-bill-transfer"></i> Withdraw</h2>
      <button class="btn btn-primary" id="withdrawBtn"><i class="fa-solid fa-arrow-up-right-from-square"></i> Withdraw</button></div>
      <div class="grid two">
        <div><b>WD Otomatis</b><p class="muted">Minimal Rp 50.000 + fee Rp 5.000. Jadi saldo minimal Rp 55.000. Masuk antrean dan menunggu konfirmasi admin.</p></div>
        <div><b>WD Instant</b><p class="muted">Rp 50k / 100k / 150k / 200k / 250k + fee Rp 15.000. Limit Rp 500.000/hari.</p></div>
      </div>
    </div>
    <div class="card" style="margin-top:15px"><h2 class="section-title"><i class="fa-solid fa-clock-rotate-left"></i> ${tr('transactions')}</h2>
      <div class="table-wrap"><table class="table"><thead><tr><th>${tr('date')}</th><th>Description</th><th>${tr('amount')}</th><th>${tr('status')}</th></tr></thead>
      <tbody>${tx?.length?tx.map(x=>`<tr><td>${new Date(x.created_at).toLocaleString()}</td><td>${escape(x.description||x.type)}</td><td class="${x.direction==='credit'?'money-plus':'money-minus'}">${x.direction==='credit'?'+':'-'}${money(x.amount)}</td><td>${escape(x.status)}</td></tr>`).join(''):`<tr><td colspan="4">${tr('noTransactions')}</td></tr>`}</tbody></table></div>
    </div>
    <div class="card" style="margin-top:15px"><h2 class="section-title">${tr('withdrawHistory')}</h2>
      <div class="table-wrap"><table class="table"><thead><tr><th>Ticket</th><th>${tr('date')}</th><th>Mode</th><th>Nominal</th><th>Fee</th><th>Total</th><th>Queue</th><th>${tr('status')}</th></tr></thead>
      <tbody>${wd?.length?wd.map(x=>`<tr><td><code>${escape(x.ticket||'-')}</code></td><td>${new Date(x.created_at).toLocaleString()}</td><td>${escape(x.withdrawal_mode||'auto')}</td><td>${money(x.requested_amount??x.amount)}</td><td>${money(x.fee||0)}</td><td>${money(x.total_debit??x.amount)}</td><td>${escape(x.queue_position??'-')}</td><td>${escape(x.status)}</td></tr>`).join(''):`<tr><td colspan="8">${tr('noWithdrawals')}</td></tr>`}</tbody></table></div>
    </div>`;
...
function openWithdraw(){
  openModal(`<div class="modal-head"><h2 class="section-title"><i class="fa-solid fa-money-bill-transfer"></i> ${tr('withdraw')}</h2><button class="close" id="x"><i class="fa-solid fa-xmark"></i></button></div>
  <form id="withdrawForm" class="grid">
    <div class="form-group"><label class="form-label">Mode WD</label>
      <select class="select" id="wdMode">
        <option value="auto">WD Otomatis — min 50k + fee 5k</option>
        <option value="instant">WD Instant — fee 15k</option>
      </select>
    </div>
    <div class="form-group"><label class="form-label">${tr('amount')}</label>
      <select class="select" id="wdAmountPreset"></select>
      <input class="input" id="wdAmount" type="number" min="50000" step="1000" required placeholder="50000" style="margin-top:8px">
      <div class="help" id="wdRule">Minimal Rp 50.000 + fee Rp 5.000 = saldo Rp 55.000.</div>
    </div>
    <div class="form-group"><label class="form-label">${tr('method')}</label><select class="select" id="wdMethod"><option value="bank">${tr('bank')}</option><option value="ewallet">${tr('ewallet')}</option><option value="crypto">${tr('crypto')}</option></select></div>
    <div class="form-group"><label class="form-label">${tr('accountName')}</label><input class="input" id="wdName" required></div>
    <div class="form-group"><label class="form-label">${tr('accountNumber')}</label><input class="input" id="wdNumber" required></div>
    <div class="actions"><button type="button" class="btn btn-secondary" id="cx">${tr('cancel')}</button><button class="btn btn-primary"><i class="fa-solid fa-paper-plane"></i> ${tr('requestWithdraw')}</button></div>
  </form>`);
  $('#x').onclick=$('#cx').onclick=closeModal;
  const sync=()=>{
    const mode=$('#wdMode').value, sel=$('#wdAmountPreset'), input=$('#wdAmount'), rule=$('#wdRule');
    if(mode==='instant'){
      sel.innerHTML='<option value="">Pilih nominal instant</option>'+[50000,100000,150000,200000,250000].map(n=>`<option value="${n}">${money(n)} + fee ${money(15000)}</option>`).join('');
      sel.style.display='block';input.style.display='none';input.required=false;
      rule.textContent='Fee Rp 15.000. Limit total nominal WD instant Rp 500.000 per hari.';
    }else{
      sel.innerHTML='';sel.style.display='none';input.style.display='block';input.required=true;
      rule.textContent='Minimal Rp 50.000 + fee Rp 5.000. Saldo minimal Rp 55.000.';
    }
  };
  $('#wdMode').onchange=sync;sync();
  $('#wdAmountPreset').onchange=()=>$('#wdAmount').value=$('#wdAmountPreset').value;
  $('#withdrawForm').onsubmit=async e=>{
    e.preventDefault();
    const mode=$('#wdMode').value;
    const amount=mode==='instant'?Number($('#wdAmountPreset').value):Number($('#wdAmount').value);
    if(!Number.isFinite(amount)||amount<=0)return toast('Nominal WD tidak valid.','error');
    const {data,error}=await sup.rpc('request_withdrawal_v2',{
      p_amount:amount,p_mode:mode,p_method:$('#wdMethod').value,
      p_account_name:$('#wdName').value.trim(),p_account_number:$('#wdNumber').value.trim()
    });
    if(error)return toast(error.message,'error');
    closeModal();
    toast(`WD dibuat. Ticket ${data.ticket}. Menunggu antrean/admin.`,'success');
    renderPayment();
  };
}
...
async function renderProfile(){setActive('profile');const p=state.profile;$('#content').innerHTML=`<div class="page-head"><div><div class="eyebrow">ACCOUNT</div><h1 class="page-title"><i class="fa-solid fa-user"></i> ${tr('profile')}</h1></div></div><div class="grid two"><section class="card"><div class="profile-head"><div class="avatar-lg">${initials(p.username)}</div><div class="profile-meta"><h2>@${escape(p.username||'user')}</h2><p>${escape(p.display_name||'TeleCod Creator')}</p></div></div><div style="margin-top:20px" class="grid"><div><div class="stat-label">${tr('username')}</div><div>${escape(p.username||'-')}</div></div><div><div class="stat-label">${tr('telegramNo')}</div><div>${escape(p.telegram_number||p.telegram_id||'-')}</div></div></div></section><section class="card"><h2 class="section-title">${tr('profileInfo')}</h2><form id="profileForm" class="grid"><div class="form-group"><label class="form-label">${tr('username')}</label><input class="input" id="pUsername" value="${escape(p.username||'')}" required></div><div class="form-group"><label class="form-label">Display Name</label><input class="input" id="pName" value="${escape(p.display_name||'')}"></div><div class="actions"><button class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> ${tr('save')}</button></div></form></section></div>`;$('#profileForm').onsubmit=async e=>{e.preventDefault();const username=$('#pUsername').value.trim().replace(/^@/,'').toLowerCase();if(!/^[a-z0-9_]{3,32}$/.test(username))return toast('Invalid username','error');const {error}=await sup.from('profiles').update({username,display_name:$('#pName').value.trim()}).eq('id',state.user.id);if(error)toast(error.message,'error');else{state.profile.username=username;state.profile.display_name=$('#pName').value.trim();toast(tr('saved'),'success');renderProfile()}}}
async function renderSettings(){setActive('settings');$('#content').innerHTML=`<div class="page-head"><div><div class="eyebrow">ACCOUNT</div><h1 class="page-title"><i class="fa-solid fa-gear"></i> ${tr('settings')}</h1></div></div><div class="grid two"><section class="card"><h2 class="section-title">${tr('appearance')}</h2><div class="toolbar"><button class="btn ${state.theme==='dark'?'btn-primary':'btn-secondary'}" id="dark"><i class="fa-solid fa-moon"></i> ${tr('dark')}</button><button class="btn ${state.theme==='light'?'btn-primary':'btn-secondary'}" id="light"><i class="fa-solid fa-sun"></i> ${tr('light')}</button></div></section><section class="card"><h2 class="section-title">${tr('language')}</h2><div class="toolbar"><button class="btn ${state.lang==='id'?'btn-primary':'btn-secondary'}" id="idLang">🇮🇩 Indonesia</button><button class="btn ${state.lang==='en'?'btn-primary':'btn-secondary'}" id="enLang">🇬🇧 English</button></div></section><section class="card"><h2 class="section-title">${tr('security')}</h2><p class="muted">Password dikelola Supabase Auth.</p><a class="btn btn-secondary" href="reset.html"><i class="fa-solid fa-key"></i> Reset Password</a></section></div>`;$('#dark').onclick=()=>{state.theme='dark';setTheme();renderSettings()};$('#light').onclick=()=>{state.theme='light';setTheme();renderSettings()};$('#idLang').onclick=()=>{state.lang='id';localStorage.setItem('telecod_lang','id');applyLang();renderSettings()};$('#enLang').onclick=()=>{state.lang='en';localStorage.setItem('telecod_lang','en');applyLang();renderSettings()}}
