/* GENERATED PAGE JS BUNDLE: create-code.html */
...
function wire(){document.querySelectorAll('input[name="access"]').forEach(r=>r.addEventListener('change',()=>{$('priceBox').hidden=access()!=='paid';$('price').disabled=access()!=='paid';if(access()!=='paid')$('price').value='0'}));$('description')?.addEventListener('input',()=>{$('counter').textContent=$('description').value.length});$('botId')?.addEventListener('change',()=>{const b=bots.find(x=>String(x.id)===$('botId').value);if(b){$('botUsernamePreview').textContent='@'+String(b.bot_username||'').replace(/^@/,'')}})}
