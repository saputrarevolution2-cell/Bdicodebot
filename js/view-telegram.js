/* PasTele — self-contained view-telegram page script. One HTML -> one JS -> one CSS. */
...
function linkifyText(value){
 ...
 return escText.replace(re,full=>{
   let x=full,tail="";
   while(/[),.!?;:'"\]]$/.test(x)){tail=x.slice(-1)+tail;x=x.slice(0,-1);}
   let u=x;
   if(/^www\./i.test(u))u="https://"+u;
   try{const parsed=new URL(u);if(!/^https?:$/.test(parsed.protocol))return full;u=parsed.href}
   catch{return full}
   return `<a class="content-link" href="${String(u).replace(/"/g,"&quot;")}" target="_blank" rel="noopener noreferrer nofollow">${x}</a>${tail}`;
 }).replace(/\r?\n/g,"<br>");
}
