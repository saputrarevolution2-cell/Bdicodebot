...
/* =========================================================
   PasTele — My Products
   DATABASE-SYNCED FINAL PAGE LOGIC
   ========================================================= */
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const esc = value => {
    if (typeof TC.esc === "function") return TC.esc(String(value ?? ""));
    return String(value ?? "").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]));
  };
  const normalize = value => String(value ?? "").trim().toLowerCase();
  const showToast = (message, type="error") => {
    if (typeof TC.toast === "function") return TC.toast(message, type);
  ...
  const iconFor = type => ({paste:"fa-file-lines",pastelink:"fa-link",code:"fa-code",channel:"fa-tower-broadcast",product:"fa-box"}[normalize(type)] || "fa-box");
  const labelFor = type => ({paste:"Paste",pastelink:"PasteLink",code:"Code Telegram",channel:"Channel / Group",product:"Marketplace Product"}[normalize(type)] || "Product");
  const typeValue = (item, fallback="") => normalize(item?.__type || item?.type || item?.product_type || fallback);
  const statusOf = item => {
    const status = normalize(item?.status);
    const price = Number(item?.price || 0);
    if (status === "draft" || status === "private" || status === "hidden") return {value:"draft",label:status === "private" ? "Private" : "Draft",icon:"fa-file-pen"};
    if (status === "inactive" || status === "disabled" || status === "archived") return {value:"draft",label:"Inactive",icon:"fa-circle-pause"};
    if (status === "published" || status === "active") return price > 0 ? {value:"paid",label:"Paid",icon:"fa-tag"} : {value:"published",label:"Published",icon:"fa-circle-check"};
    return {value:"draft",label:"Draft",icon:"fa-file-pen"};
  };
  ...
  function renderLoading(){
    content.innerHTML=`<div class="loading-state"><div class="loading-spinner"><i class="fa-solid fa-spinner fa-spin"></i></div><strong>Memuat produk...</strong><span>Mengambil data terbaru dari database.</span></div>`;
    if(resultInfo) resultInfo.textContent="Memuat produk...";
  }
  function renderError(message){
    content.innerHTML=`<div class="error-state"><div class="error-state-icon"><i class="fa-solid fa-triangle-exclamation"></i></div><strong>Gagal memuat produk</strong><span>${esc(message || "Terjadi kesalahan saat mengambil data.")}</span><button class="btn primary" id="retryBtn" type="button"><i class="fa-solid fa-rotate"></i> Coba Lagi</button></div>`;
    if(resultInfo) resultInfo.textContent="Gagal memuat data.";
    $("retryBtn")?.addEventListener("click", loadData);
  }

  function updateOverview(){
    const total=allItems.length;
    const published=allItems.filter(x=>statusOf(x).value==="published").length;
    const paid=allItems.filter(x=>statusOf(x).value==="paid").length;
    const draft=allItems.filter(x=>statusOf(x).value==="draft").length;
  ...
  async function loadData(){
    ...
    refreshBtn?.classList.add("is-loading"); refreshBtn && (refreshBtn.disabled=true);
    try{
      ...
      const pasteItems=(pastes.data||[]).map(x=>({...x,__type:"paste",status:x.visibility==="public"?"published":"draft",price:0,access_type:"free"}));
      const codeItems=(codes.data||[]).map(x=>({...x,__type:"code"}));
      const channelItems=(channels.data||[]).map(x=>({...x,__type:"channel"}));

      groups=[
        {key:"pastelink",title:"PasteLink",icon:"fa-link",items:pastelinkItems},
        {key:"paste",title:"Paste",icon:"fa-file-lines",items:pasteItems},
        {key:"code",title:"Code Telegram",icon:"fa-code",items:codeItems},
        {key:"channel",title:"Channel / Group",icon:"fa-tower-broadcast",items:channelItems},
        {key:"product",title:"Marketplace Product",icon:"fa-box",items:productItems}
      ];
      allItems=groups.flatMap(g=>g.items);
      updateOverview(); render();
    }catch(e){
      console.error("[My Products] Load error",e); renderError(e?.message || "Tidak dapat mengambil data dari database.");
    }finally{
      loading=false; refreshBtn && (refreshBtn.disabled=false); refreshBtn?.classList.remove("is-loading");
    }
  }
  ...
  function pageFor(key){ const n=Number(groupPages[key]||1); return Number.isFinite(n)&&n>0?Math.floor(n):1; }
  function setGroupPage(key,page){ groupPages[key]=Math.max(1,Math.floor(Number(page)||1)); }

  function render(){
    const filtered=getFilteredGroups();
    const visible=filtered.reduce((n,g)=>n+g.items.length,0);
    const hasFilter=!!String(searchInput?.value||"").trim() || (typeFilter?.value||"all")!=="all" || (statusFilter?.value||"all")!=="all";
    if(resultInfo) resultInfo.textContent=hasFilter?`${visible} hasil ditemukan`:`${allItems.length} konten tersedia`;
    clearSearch?.classList.toggle("hidden",!String(searchInput?.value||"").trim());
    const html=filtered.filter(g=>g.items.length).map(renderGroup).join("");
    if(html){content.innerHTML=html;bindPagination();bindExpanders();bindActions();return;}
    const empty=allItems.length===0;
    content.innerHTML=`<div class="empty-state"><div class="empty-state-icon"><i class="fa-solid ${empty?'fa-box-open':'fa-magnifying-glass'}"></i></div><strong>${empty?'Belum ada produk':'Produk tidak ditemukan'}</strong><span>${empty?'Buat PasteLink, Code Telegram, Channel, atau produk marketplace pertamamu.':'Coba ubah kata kunci atau filter pencarian.'}</span>${empty?'<a href="create-product.html" class="btn primary"><i class="fa-solid fa-plus"></i> Buat Produk</a>':'<button class="btn" id="resetFilterBtn" type="button"><i class="fa-solid fa-filter-circle-xmark"></i> Reset Filter</button>'}</div>`;
    $("resetFilterBtn")?.addEventListener("click",resetFilters);
  }

  function renderGroup(group){
    const total=group.items.length, pages=Math.max(1,Math.ceil(total/ITEMS_PER_PAGE));
    const current=Math.min(pageFor(group.key),pages); setGroupPage(group.key,current);
    const start=(current-1)*ITEMS_PER_PAGE, pageItems=group.items.slice(start,start+ITEMS_PER_PAGE);
    const pagination=pages>1?`<nav class="my-pagination" aria-label="Halaman ${esc(group.title)}"><button class="page-btn" type="button" data-page-action="prev" data-page-group="${esc(group.key)}" ${current===1?'disabled':''} aria-label="Sebelumnya"><i class="fa-solid fa-chevron-left"></i></button>${Array.from({length:pages},(_,i)=>{const p=i+1;return `<button class="page-btn ${p===current?'active':''}" type="button" data-page-action="goto" data-page-group="${esc(group.key)}" data-page="${p}" aria-current="${p===current?'page':'false'}">${p}</button>`}).join("")}<button class="page-btn" type="button" data-page-action="next" data-page-group="${esc(group.key)}" ${current===pages?'disabled':''} aria-label="Berikutnya"><i class="fa-solid fa-chevron-right"></i></button></nav>`:"";
    return `<section class="my-section"><div class="my-section-header"><div class="my-section-title"><span class="my-section-title-icon"><i class="fa-solid ${group.icon}"></i></span><div><h2>${esc(group.title)}</h2><span class="my-section-subtitle">Kelola konten ${esc(group.title)}</span></div></div><span class="my-section-count">${group.items.length}</span></div><div class="my-list">${pageItems.map(item=>{
  ...
  function bindPagination(){
    content.querySelectorAll("[data-page-action]").forEach(btn=>btn.addEventListener("click",()=>{
      const key=btn.dataset.pageGroup, group=getFilteredGroups().find(g=>g.key===key); if(!group)return;
      const pages=Math.max(1,Math.ceil(group.items.length/ITEMS_PER_PAGE)); let page=pageFor(key);
      if(btn.dataset.pageAction==="prev")page--; else if(btn.dataset.pageAction==="next")page++; else page=Number(btn.dataset.page||1);
      setGroupPage(key,Math.max(1,Math.min(pages,page))); render();
    }));
  }
  ...
  const validPrice = value => { const n=Number(value); if(!Number.isFinite(n)||n===0)return 0; if(n<2000||n>100000||n%1000!==0) throw new Error("Harga berbayar harus Rp2.000–Rp100.000 dan kelipatan Rp1.000."); return n; };
  const localDateTimeToISO = value => { if(!value)return null; const d=new Date(value); if(Number.isNaN(d.getTime()))throw new Error("Tanggal expired tidak valid."); return d.toISOString(); };
  const toLocalDateTimeValue = value => { if(!value)return "";const d=new Date(value);if(Number.isNaN(d.getTime()))return "";const p=n=>String(n).padStart(2,"0");return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}T${p(d.getHours())}:${p(d.getMinutes())}`; };
  function removeEditModal(){document.getElementById("ptEditModal")?.remove();document.getElementById("ptEditBackdrop")?.remove();document.body.classList.remove("modal-open");document.documentElement.classList.remove("modal-open","modal-blur");}

  function editModalTemplate(item,type){
    const title=titleOf(item), heading=type==="code"?"Edit Code Telegram":type==="pastelink"?"Edit PasteLink":type==="channel"?`Edit ${normalize(item?.type)==="group"?"Group":"Channel"}`:type==="paste"?"Edit Paste":"Edit Marketplace Product";
    let fields=`<div class="edit-field"><label for="editTitle"><i class="fa-solid fa-heading"></i> Judul</label><input id="editTitle" class="edit-input" maxlength="180" value="${esc(title)}" autocomplete="off"></div>`;
    if(type==="code") fields+=`<div class="edit-field"><label for="editBot"><i class="fa-solid fa-robot"></i> Bot Telegram</label><input id="editBot" class="edit-input" maxlength="120" value="${esc(item?.bot_username?"@"+String(item.bot_username).replace(/^@/,""):"")}" placeholder="@NamaBot"></div><div class="edit-field"><label for="editCode"><i class="fa-solid fa-key"></i> Code / Slug</label><input id="editCode" class="edit-input" maxlength="180" value="${esc(item?.slug||"")}"><small class="edit-help">Slug adalah code publik dan harus unik.</small></div><div class="edit-field"><label for="editCodeDesc"><i class="fa-solid fa-align-left"></i> Deskripsi</label><textarea id="editCodeDesc" class="edit-textarea" rows="5">${esc(item?.description||"")}</textarea></div>`;
    else if(type==="pastelink") fields+=`<div class="edit-field"><label for="editContent"><i class="fa-solid fa-file-code"></i> Konten HTML</label><textarea id="editContent" class="edit-textarea edit-content" rows="14">${esc(item?.content_html||"")}</textarea></div><div class="edit-field"><label for="editExpires"><i class="fa-solid fa-clock"></i> Expired</label><div class="edit-expire-row"><input id="editExpires" class="edit-input" type="datetime-local" value="${esc(toLocalDateTimeValue(item?.expires_at))}"><label class="edit-check"><input id="editNever" type="checkbox" ${item?.expires_at?"":"checked"}><span>Tidak expired</span></label></div></div><div class="edit-field"><label for="editVisibility"><i class="fa-solid fa-eye"></i> Visibility</label><select id="editVisibility" class="edit-input"><option value="public" ${item?.visibility==="public"?"selected":""}>Public</option><option value="private" ${item?.visibility!=="public"?"selected":""}>Private</option></select></div>`;
    else if(type==="paste") fields+=`<div class="edit-field"><label for="editPasteSlug"><i class="fa-solid fa-link"></i> Slug</label><input id="editPasteSlug" class="edit-input" maxlength="180" value="${esc(item?.slug||"")}"></div><div class="edit-field"><label for="editContent"><i class="fa-solid fa-file-lines"></i> Konten</label><textarea id="editContent" class="edit-textarea" rows="14">${esc(item?.content||"")}</textarea></div><div class="edit-field"><label for="editVisibility"><i class="fa-solid fa-eye"></i> Visibility</label><select id="editVisibility" class="edit-input"><option value="public" ${item?.visibility==="public"?"selected":""}>Public</option><option value="private" ${item?.visibility!=="public"?"selected":""}>Private</option></select></div>`;
    else if(type==="channel") fields+=`<div class="edit-field"><label for="editUsername"><i class="fa-solid fa-at"></i> Username Telegram</label><input id="editUsername" class="edit-input" maxlength="120" value="${esc(item?.username?"@"+String(item.username).replace(/^@/,""):"")}" placeholder="@channel"></div><div class="edit-field"><label for="editLink"><i class="fa-solid fa-link"></i> Link Channel / Group</label><input id="editLink" class="edit-input" maxlength="1000" value="${esc(item?.invite_url||"")}" placeholder="https://t.me/..."></div><div class="edit-field"><label for="editChannelDesc"><i class="fa-solid fa-align-left"></i> Deskripsi</label><textarea id="editChannelDesc" class="edit-textarea" rows="5">${esc(item?.description||"")}</textarea></div>`;
    else fields+=`<div class="edit-field"><label for="editSlug"><i class="fa-solid fa-link"></i> Slug</label><input id="editSlug" class="edit-input" maxlength="180" value="${esc(item?.slug||"")}"></div><div class="edit-field"><label for="editDescription"><i class="fa-solid fa-align-left"></i> Deskripsi</label><textarea id="editDescription" class="edit-textarea" rows="7">${esc(item?.description||"")}</textarea></div><div class="edit-field"><label for="editPrice"><i class="fa-solid fa-tag"></i> Harga (IDR)</label><input id="editPrice" class="edit-input" inputmode="numeric" value="${esc(String(item?.price||0))}"><small class="edit-help">Gratis = Rp0. Paid = Rp2.000–Rp100.000, kelipatan Rp1.000.</small></div>`;
    return `<div class="pt-edit-backdrop" id="ptEditBackdrop"></div><section class="pt-edit-modal" id="ptEditModal" role="dialog" aria-modal="true"><header class="pt-edit-head"><div class="pt-edit-heading"><span class="pt-edit-icon"><i class="fa-solid ${iconFor(type)}"></i></span><div><span class="pt-edit-eyebrow">${esc(labelFor(type))}</span><h2>${esc(heading)}</h2></div></div><button type="button" class="pt-edit-close" id="ptEditClose"><i class="fa-solid fa-xmark"></i></button></header><form id="ptEditForm" class="pt-edit-form">${fields}<div id="ptEditError" class="edit-error" hidden></div><footer class="pt-edit-actions"><button type="button" class="btn" id="ptEditCancel"><i class="fa-solid fa-xmark"></i> Batal</button><button type="submit" class="btn primary" id="ptEditSave"><i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan</button></footer></form></section>`;
  }

  async function editItem(item,type){
    if(!profile?.id)return showToast("Sesi login tidak tersedia.");
    removeEditModal();document.body.insertAdjacentHTML("beforeend",editModalTemplate(item,type));document.body.classList.add("modal-open");
    const form=$("ptEditForm"), save=$("ptEditSave"), errorBox=$("ptEditError"), close=removeEditModal;
    $("ptEditClose")?.addEventListener("click",close);$("ptEditCancel")?.addEventListener("click",close);$("ptEditBackdrop")?.addEventListener("click",close);
    const never=$("editNever"),expires=$("editExpires");never?.addEventListener("change",()=>{expires.disabled=never.checked;if(never.checked)expires.value="";});if(never?.checked&&expires)expires.disabled=true;
    form?.addEventListener("submit",async e=>{e.preventDefault();if(save.disabled)return;errorBox.hidden=true;save.disabled=true;save.classList.add("is-loading");try{
      const title=String($("editTitle")?.value||"").trim();if(!title)throw new Error("Judul wajib diisi.");let q;
      if(type==="code"){const code=String($("editCode")?.value||"").trim(),bot=String($("editBot")?.value||"").trim().replace(/^@/,"");if(!/^[A-Za-z0-9._-]{3,180}$/.test(code))throw new Error("Code tidak valid.");q=await supabase.from("telegram_products").update({title,slug:code,bot_username:bot||null,description:String($("editCodeDesc")?.value||"").trim(),updated_at:new Date().toISOString()}).eq("id",item.id).eq("owner_id",profile.id);}
      else if(type==="pastelink"){const html=String($("editContent")?.value||"");if(!html.trim())throw new Error("Konten PasteLink wajib diisi.");let exp=null;if(!never?.checked&&expires?.value){exp=localDateTimeToISO(expires.value);if(new Date(exp)<=new Date())throw new Error("Tanggal expired harus di masa depan.");}q=await supabase.from("pastelinks").update({title,content_html:html,expires_at:exp,visibility:$("editVisibility")?.value==="public"?"public":"private",updated_at:new Date().toISOString()}).eq("id",item.id).eq("user_id",profile.id);}
      else if(type==="paste"){const slug=String($("editPasteSlug")?.value||"").trim(),body=String($("editContent")?.value||"");if(!/^[A-Za-z0-9._-]{3,180}$/.test(slug))throw new Error("Slug tidak valid.");if(!body.trim())throw new Error("Konten wajib diisi.");q=await supabase.from("pastes").update({title,slug,content:body,visibility:$("editVisibility")?.value==="public"?"public":"private",updated_at:new Date().toISOString()}).eq("id",item.id).eq("owner_id",profile.id);}
      else if(type==="channel"){let username=String($("editUsername")?.value||"").trim().replace(/^@/,""),link=String($("editLink")?.value||"").trim();if(link&&!/^https?:\/\//i.test(link)&&/^t\.me\//i.test(link))link="https://"+link;q=await supabase.from("telegram_channels").update({name:title,username:username||null,invite_url:link||null,description:String($("editChannelDesc")?.value||"").trim(),updated_at:new Date().toISOString()}).eq("id",item.id).eq("owner_id",profile.id);}
      else {const slug=String($("editSlug")?.value||"").trim(),description=String($("editDescription")?.value||"").trim(),price=validPrice(String($("editPrice")?.value||"0").replace(/[^\d]/g,""));if(!/^[A-Za-z0-9._-]{3,180}$/.test(slug))throw new Error("Slug tidak valid.");q=await supabase.from("products").update({title,slug,description,price,access_type:price>0?"paid":"free",updated_at:new Date().toISOString()}).eq("id",item.id).or(`creator_id.eq.${profile.id},seller_id.eq.${profile.id}`);}
      if(q?.error)throw q.error;removeEditModal();showToast("Perubahan berhasil disimpan.","success");await loadData();
    }catch(err){errorBox.textContent=err?.message||"Gagal menyimpan perubahan.";errorBox.hidden=false;}finally{save.disabled=false;save.classList.remove("is-loading");}});
  }

  async function deleteItem(item,type){
    if(!confirm(`Hapus "${titleOf(item)}"?\n\nTindakan ini tidak dapat dibatalkan.`))return;
    let q;
    if(type==="pastelink")q=await supabase.from("pastelinks").delete().eq("id",item.id).eq("user_id",profile.id);
    else if(type==="paste")q=await supabase.from("pastes").delete().eq("id",item.id).eq("owner_id",profile.id);
    else if(type==="code")q=await supabase.from("telegram_products").delete().eq("id",item.id).eq("owner_id",profile.id);
    else if(type==="channel")q=await supabase.from("telegram_channels").delete().eq("id",item.id).eq("owner_id",profile.id);
    else q=await supabase.from("products").delete().eq("id",item.id).or(`creator_id.eq.${profile.id},seller_id.eq.${profile.id}`);
    if(q?.error){showToast(q.error.message||"Gagal menghapus konten.");return;}showToast("Konten berhasil dihapus.","success");await loadData();
  }

  function resetFilters(){if(searchInput)searchInput.value="";if(typeFilter)typeFilter.value="all";if(statusFilter)statusFilter.value="all";render();}
  searchInput?.addEventListener("input",render);typeFilter?.addEventListener("change",render);statusFilter?.addEventListener("change",render);clearSearch?.addEventListener("click",()=>{searchInput.value="";searchInput.focus();render();});refreshBtn?.addEventListener("click",loadData);
  await loadData();
});
