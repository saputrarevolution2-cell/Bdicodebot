...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function render(){
    ...
    if(html){content.innerHTML=html;bindExpanders();bindActions();return;}
  ...
  function bindActions(){content.querySelectorAll("[data-action]").forEach(btn=>btn.addEventListener("click",async e=>{e.preventDefault();e.stopPropagation();if(btn.disabled)return;const item=findItem(btn.dataset.id,btn.dataset.type);if(!item)return showToast("Data tidak ditemukan. Silakan refresh.");btn.disabled=true;try{if(btn.dataset.action==="open")openItem(item,btn.dataset.type);else if(btn.dataset.action==="copy")await copyItem(item,btn.dataset.type);else if(btn.dataset.action==="edit")await editItem(item,btn.dataset.type);else if(btn.dataset.action==="delete")await deleteItem(item,btn.dataset.type);}catch(err){showToast(err?.message||"Action gagal.");}finally{if(document.body.contains(btn))btn.disabled=false;}}));}
