/* PasTele — self-contained payment-success page script. One HTML -> one JS -> one CSS. */
...
document.addEventListener(
    ...
    async () => {
        ...
        /* ===================================================
           ...
           =================================================== */
        try {
            const { data: order, error } = await window.PasTeleDB.rpc("get_order_for_payment", { p_order_id: orderId, p_guest_token: guestToken || null });
            ...
            const canonicalView = async (type, id) => {
                ...
                try {
                    ...
                    const rpc = token
                        ? await window.PasTeleDB.rpc("get_market_item_detail_guest", {
                            p_type: rpcType, p_id: id, p_guest_token: token
                        })
                        : await window.PasTeleDB.rpc("get_market_item_detail", {
