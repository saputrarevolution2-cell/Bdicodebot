...
$("createForm")?.addEventListener("submit",async e=>{e.preventDefault();if($("submitBtn").disabled)return;const v=validate();if(!v)return;setLoading(true);try{requireClient();if(v.a==="paid"&&!(await window.TC?.user?.())){toast("Konten Paid hanya bisa dibuat setelah login atau daftar akun.","error");const next=`${location.pathname}${location.search}${location.hash}`;const loginUrl=`login.html?next=${encodeURIComponent(next)}&reason=paid-create`;setLoading(false);window.setTimeout(()=>window.location.assign(loginUrl),450);return;}const slug=await createUniqueShortCode();const {data,error}=await sb().rpc("create_telegram_content",{p_name:v.title,p_slug:slug,p_type:kind(),p_access_type:v.a,p_price:v.price,p_description:v.desc,p_username:v.tg.username,p_invite_url:v.tg.url,p_telegram_channel_id:v.raw});if(error)throw error;if(!data?.ok)throw new Error("Database tidak mengonfirmasi pembuatan Telegram content.");finish(v,data.slug||slug);$("createForm").reset();sync();toast((kind()==="group"?"Group":"Channel")+" berhasil dipublikasikan.","success")}catch(e){console.error(e);toast(e?.code==="23505"?"Slug sudah digunakan. Silakan coba lagi.":e?.message||"Gagal menyimpan Telegram.","error")}finally{setLoading(false)}});sync();
});
