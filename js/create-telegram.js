/* PasTele — self-contained create-telegram page script. One HTML -> one JS -> one CSS. */
...
async function createUniqueShortCode(client){for(let attempt=0;attempt<24;attempt++){const key=randomShortCode();const checks=await Promise.all([window.PasTeleDB.rpc("get_telegram_content_by_slug",{p_slug:key,p_type:"channel"}),window.PasTeleDB.rpc("get_telegram_content_by_slug",{p_slug:key,p_type:"group"}),window.PasTeleDB.rpc("get_code_by_slug",{p_slug:key}),window.PasTeleDB.rpc("get_pastelink_by_slug",{p_slug:key})]);if(checks.every(q=>!q?.error&&!((Array.isArray(q.data)?q.data[0]:q.data)?.found)))return key}throw new Error("Gagal membuat kode publik unik. Silakan coba lagi")}
...
function validate(){const title=$("title").value.trim(),desc=$("description").value.trim(),raw=$("telegram").value.trim(),a=access(),price=a==="paid"?Number($("price").value||0):0;if(title.length<2||title.length>120)return toast("Judul harus 2–120 karakter.","error"),null;if(!raw)return toast("Link Group/Channel wajib diisi.","error"),null;const tg=parseTelegram(raw);if(!tg.url)return toast("Masukkan username @username atau link https://t.me/... yang valid.","error"),null;if(a==="paid"&&(!Number.isInteger(price)||price<2000||price>100000||price%1000))return toast("Harga Paid harus Rp2.000–Rp100.000 dan kelipatan Rp1.000.","error"),null;return{title,desc,raw,tg,a,price}}
function finish(v,slug){const prefix=kind()==="group"?(v.a==="paid"?"gp":"gf"):(v.a==="paid"?"ch/p":"ch/f"),url=`${location.origin}/${prefix}/${encodeURIComponent(slug)}`,r=$("result");r.hidden=false;r.innerHTML=`<div class="result-icon"><i class="fa-solid fa-circle-check"></i></div><h2>${kind()==="group"?"Group":"Channel"} berhasil dipublikasikan</h2><p><b>${esc(v.title)}</b> siap dibagikan.</p><div class="result-url"><input readonly value="${esc(url)}"><button id="copyUrl" type="button"><i class="fa-regular fa-copy"></i></button></div><div class="result-actions"><a class="btn primary" href="${esc(url)}">Buka</a><a class="btn secondary" href="my-products.html">Konten Saya</a></div>`;$("copyUrl").onclick=async()=>{try{await navigator.clipboard.writeText(url);toast("Link berhasil disalin.","success")}catch{toast("Gagal menyalin link.","error")}};r.scrollIntoView({behavior:"smooth",block:"center"})}
...
$("createForm")?.addEventListener("submit",async e=>{e.preventDefault();if($("submitBtn").disabled)return;const v=validate();if(!v)return;setLoading(true);try{requireClient();if(v.a==="paid"&&!(await window.TC?.user?.())){toast("Konten Paid hanya bisa dibuat setelah login atau daftar akun.","error");const next=`${location.pathname}${location.search}${location.hash}`;const loginUrl=`login.html?next=${encodeURIComponent(next)}&reason=paid-create`;setLoading(false);window.setTimeout(()=>window.location.assign(loginUrl),450);return;}const slug=await createUniqueShortCode(sb());const {data,error}=await sb().rpc("create_telegram_content",{p_name:v.title,p_slug:slug,p_type:kind(),p_access_type:v.a,p_price:v.price,p_description:v.desc,p_username:v.tg.username,p_invite_url:v.tg.url,p_telegram_channel_id:v.raw});if(error)throw error;if(!data?.ok)throw new Error("Database tidak mengonfirmasi pembuatan Telegram content.");finish(v,data.slug||slug);$("createForm").reset();sync();toast((kind()==="group"?"Group":"Channel")+" berhasil dipublikasikan.","success")}catch(e){console.error(e);toast(e?.code==="23505"?"Slug sudah digunakan. Silakan coba lagi.":e?.message||"Gagal menyimpan Telegram.","error")}finally{setLoading(false)}});sync();
});
