...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 if(!/^[A-Za-z0-9_]{3,32}$/.test(username))return show(err,'Username hanya boleh berisi huruf, angka, dan underscore.');if(password!==confirm)return show(err,'Konfirmasi kata sandi tidak cocok.'); if(password.length<6)return show(err,'Kata sandi minimal 6 karakter.');if(!window.sb)return show(err,'Supabase belum dikonfigurasi. Buka setup.html untuk menghubungkan database.');
