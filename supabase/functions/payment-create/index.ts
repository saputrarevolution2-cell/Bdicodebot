...
Deno.serve(async req=>{
 ...
 try{
  ...
  let amount=0,payment:any,item:any;

  if(action==="purchase"){
    const {data:p,error}=await db.from("products")
      .select("id,creator_id,title,price,access_type,status,type").eq("id",b.product_id).single();
    ...
    if(user){
      ...
      const r=await db.from("purchases").insert({
        product_id:p.id,buyer_id:null,guest_token:token,amount:p.price,status:"pending"
      }).select().single();
      if(r.error)throw r.error;
      purchase=r.data;
    }
    ...
    item={name:p.title,quantity:1,price:amount};
    const ref=`TC-P-${Date.now()}-${crypto.randomUUID().slice(0,8)}`;
    const r=await db.from("payments").insert({
      user_id:user?.id||null,purchase_id:purchase.id,guest_token:purchase.guest_token||null,
      kind:"purchase",provider:"dompetx",order_id:ref,amount,currency:"IDR",status:"pending"
    }).select().single();
    if(r.error)throw r.error;
    payment=r.data;
    ...
    const r=await db.from("payments").insert({
      user_id:user.id,kind:"deposit",provider:"dompetx",order_id:ref,amount,currency:"IDR",status:"pending"
    }).select().single();
    if(r.error)throw r.error;
    payment=r.data;item={name:"TeleCod Wallet Deposit",quantity:1,price:amount};
    await db.from("transactions").insert({
      user_id:user.id,type:"deposit",direction:"credit",amount,status:"pending",
      reference_id:payment.id,description:"Wallet deposit"
    });
  ...
  const payload={
    ...
    metadata:{
      order_name:action==="purchase"?"TeleCod Product Purchase":"TeleCod Wallet Deposit",
      product_name:item.name,
      customer_name:user?.user_metadata?.username||"TeleCod Guest",
  ...
  return json({
    ok:true,payment_id:payment.id,order_id:payment.order_id,provider_payment_id:providerId,
    amount,method,guest_token:payment.guest_token||null,
    qr_url:`${base}/v1/qr/${encodeURIComponent(providerId)}`
  });
 }catch(e){
  console.error(e);
  return json({error:e instanceof Error?e.message:"Payment creation failed"},500);
 }
});