-- PasTele / Bayar.gg FULL FINAL MIGRATION — safe rerun
-- Uses IF NOT EXISTS for schema objects; functions are recreated safely because PostgreSQL has no CREATE FUNCTION IF NOT EXISTS.
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='orders' AND column_name='item_id' AND data_type='text') THEN
    ALTER TABLE public.orders ALTER COLUMN item_id TYPE uuid USING CASE WHEN item_id IS NULL OR btrim(item_id)='' THEN NULL ELSE item_id::uuid END;
  ELSE
    ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS item_id uuid;
  END IF;
EXCEPTION WHEN invalid_text_representation THEN
  RAISE EXCEPTION 'orders.item_id contains non-UUID values; clean those values before changing the column to uuid.';
END $$;
...
DECLARE uid uuid:=auth.uid(); seller uuid; title text; price numeric; oid uuid; normalized text:=lower(btrim(coalesce(p_type,'')));
BEGIN
 ...
 IF oid IS NULL THEN INSERT INTO public.orders(buyer_id,seller_id,product_id,amount,status,item_type,item_id,item_title) VALUES(uid,seller,p_id::uuid,price,'pending',normalized,p_id::uuid,title) RETURNING id INTO oid; END IF;
...
RETURNS TABLE(order_id uuid,amount numeric,item_title text,item_type text)
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE uid uuid:=auth.uid(); oid uuid; expected numeric; title text; normalized text:=lower(btrim(coalesce(p_plan,'')));
BEGIN
 IF uid IS NULL THEN RAISE EXCEPTION 'LOGIN_REQUIRED'; END IF;
 ...
 INSERT INTO public.orders(buyer_id,seller_id,product_id,amount,status,item_type,item_id,item_title) VALUES(uid,uid,NULL,expected,'pending','account_plan',NULL,title) RETURNING id INTO oid;
...
BEGIN
 ...
 IF lower(coalesce(o.item_type,''))='account_plan' THEN
   plan:=CASE
     WHEN lower(coalesce(o.item_title,''))='pastele premium' THEN 'premium'
     WHEN lower(coalesce(o.item_title,'')) LIKE 'langganan pastele 1 hari%' THEN 'subscription_1'
     WHEN lower(coalesce(o.item_title,'')) LIKE 'langganan pastele 3 hari%' THEN 'subscription_3'
     WHEN lower(coalesce(o.item_title,'')) LIKE 'langganan pastele 7 hari%' THEN 'subscription_7'
     ELSE '' END;
