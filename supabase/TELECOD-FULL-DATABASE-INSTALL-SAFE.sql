-- ============================================================
-- TELECOD FULL DATABASE INSTALL — SAFE / RE-RUNNABLE
-- Generated from the SQL files shipped with TeleCod-MASTER-CONTROL-FULL-FIX.zip
--
-- Tujuan:
--   * Bisa dijalankan pada database TeleCod yang masih kosong maupun
--     yang sebagian schema-nya sudah ada.
--   * CREATE TABLE/INDEX/EXTENSION memakai IF NOT EXISTS jika tersedia.
--   * Policy/trigger/constraint yang memang didefinisikan ulang akan
--     di-drop lalu dibuat ulang agar tidak gagal karena sudah ada.
--   * Function memakai CREATE OR REPLACE sesuai migration sumber.
--
...
drop trigger if exists profiles_updated on public.profiles;
create trigger profiles_updated before update on public.profiles for each row execute function public.set_updated_at()public.set_updated_at();

drop trigger if exists products_updated on public.products;
drop trigger if exists products_updated on public.products;
create trigger products_updated before update on public.products for each row execute function public.set_updated_at()public.set_updated_at();

drop trigger if exists wallet_updated on public.wallets;
drop trigger if exists wallet_updated on public.wallets;
create trigger wallet_updated before update on public.wallets for each row execute function public.set_updated_at()public.set_updated_at();
...
drop trigger if exists on_auth_user_created on auth.users;
create trigger on_auth_user_created after insert on auth.users for each row execute function public.handle_new_user()public.handle_new_user();
...
drop trigger if exists guard_master_admin_id_trigger on public.profiles;
create trigger guard_master_admin_id_trigger before insert or update of telegram_id on public.profiles
for each row execute function public.guard_master_admin_id()public.guard_master_admin_id();
...
-- ============================================================
-- [2/7] telecod_final_auth_marketplace.sql
-- ============================================================
-- ============================================================
-- TELECOD FINAL AUTH + MARKETPLACE RULES (append-safe)
-- ============================================================
...
-- ============================================================
-- [3/7] final_marketplace_wallet_migration.sql
-- ============================================================
-- ============================================================
-- TELECOD MARKETPLACE + WALLET FINAL FIX
-- Run after the original schema.sql on an existing Supabase DB.
...
-- ============================================================
-- [4/7] telegram_marketplace.sql
-- ============================================================
create table if not exists public.telegram_products(id uuid primary key default gen_random_uuid(),owner_id uuid references auth.users(id) on delete set null,title text not null,description text,product_type text not null default 'code',access_type text not null default 'free',price bigint not null default 0,bot_username text,telegram_bot_id bigint,is_published boolean not null default false,created_at timestamptz not null default now());
create table if not exists public.telegram_channels(id uuid primary key default gen_random_uuid(),owner_id uuid references auth.users(id) on delete set null,title text not null,access_type text not null default 'free',price bigint not null default 0,telegram_channel_id text,is_published boolean not null default false,created_at timestamptz not null default now());
create table if not exists public.bot_integrations(id uuid primary key default gen_random_uuid(),owner_id uuid references auth.users(id) on delete set null,bot_username text,telegram_bot_id bigint,encrypted_token text,is_active boolean not null default false,created_at timestamptz not null default now(),updated_at timestamptz not null default now());
create index if not exists telegram_products_market_idx on public.telegram_products(is_published,access_type,created_at desc);


-- ============================================================
-- [5/7] marketplace_fee.sql
-- ============================================================
...
-- ============================================================
-- [6/7] notifications.sql
-- ============================================================
create table if not exists public.notifications(id uuid primary key default gen_random_uuid(),user_id uuid not null references auth.users(id) on delete cascade,title text not null,message text not null,type text not null default 'info',is_read boolean not null default false,created_at timestamptz not null default now()); create index if not exists notifications_user_created_idx on public.notifications(user_id,created_at desc); alter table public.notifications enable row level security;


-- ============================================================
-- [7/7] admin_control_center.sql
-- ============================================================
...
-- Basic post-install checks (tidak mengubah data)
do $$
begin
  if to_regclass('public.profiles') is null then
    raise exception 'TeleCod install failed: public.profiles is missing';
  end if;
  if to_regclass('public.products') is null then
    raise exception 'TeleCod install failed: public.products is missing';
  end if;
  if to_regclass('public.wallets') is null then
    raise exception 'TeleCod install failed: public.wallets is missing';
  end if;
end $$;
