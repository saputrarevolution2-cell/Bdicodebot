...
begin
  insert into public.profiles(
    id,username,telegram_id,telegram_username,telegram_number,display_name,avatar_url
  ) values(
    new.id,
...
-- Existing users: create wallet only when missing.
insert into public.wallets(user_id)
...
-- ============================================================
-- 17. PUBLIC MARKETPLACE OBJECT (TABLE/VIEW SAFE)
-- ============================================================
-- IMPORTANT:
-- * Never drops public.marketplace_public when it is an existing table.
-- * If it is already a view, it is replaced safely.
-- * If it does not exist, a live view is created.
-- * If it is an existing table, the table is preserved and synchronized
--   through a trigger. Existing rows are NOT deleted.
do $$
declare
  relkind "char";
  rel_exists boolean;
begin
  select exists(
    ...
    where n.nspname='public' and c.relname='marketplace_public'
  ) into rel_exists;
  ...
  if not rel_exists then
    ...
    if relkind='v' then
      ...
      -- Existing table: preserve it and add only the missing compatibility
      -- column. The table is NOT dropped.
      execute 'alter table public.marketplace_public add column if not exists creator_username text';

      execute $v$
        create or replace function public.sync_marketplace_public_row()
        returns trigger
        language plpgsql
        security definer
        set search_path=public
        ...
        begin
          if tg_op='DELETE' then
            delete from public.marketplace_public where id=old.id;
            return old;
          end if;

          if new.status='published' then
            insert into public.marketplace_public(
              id,creator_id,type,title,slug,description,category,access_type,
              price,thumbnail_url,telegram_channel,is_channel,status,views,
              sales_count,created_at,updated_at,creator_username
            )
            select
              new.id,new.creator_id,new.type,new.title,new.slug,new.description,
              new.category,new.access_type,new.price,new.thumbnail_url,
              new.telegram_channel,new.is_channel,new.status,new.views,
              new.sales_count,new.created_at,new.updated_at,
              coalesce(pr.username,'TeleCod')
            ...
            -- Handle products whose creator_id is NULL.
            insert into public.marketplace_public(
              id,creator_id,type,title,slug,description,category,access_type,
              price,thumbnail_url,telegram_channel,is_channel,status,views,
              sales_count,created_at,updated_at,creator_username
            ...
            select
              ...
              new.category,new.access_type,new.price,new.thumbnail_url,
              new.telegram_channel,new.is_channel,new.status,new.views,
              new.sales_count,new.created_at,new.updated_at,'TeleCod'
            where new.creator_id is null
            on conflict (id) do update set
              title=excluded.title,
              slug=excluded.slug,
              description=excluded.description,
            ...
            delete from public.marketplace_public where id=new.id;
          end if;
      ...
      drop trigger if exists products_sync_marketplace_public
        on public.products;
      create trigger products_sync_marketplace_public
      after insert or update or delete on public.products
      for each row execute function public.sync_marketplace_public_row();

      -- Backfill only missing rows. Existing marketplace rows are preserved.
      insert into public.marketplace_public(
        id,creator_id,type,title,slug,description,category,access_type,
        price,thumbnail_url,telegram_channel,is_channel,status,views,
        sales_count,created_at,updated_at,creator_username
...
-- ============================================================
-- PAYMENT COMPATIBILITY: RELEASE MATURED SALES
-- ============================================================
-- The ZIP's payment-status Edge Function calls this RPC. It releases
-- pending seller transactions after 24 hours without deleting data.
...
grant execute on function public.release_matured_sales() to service_role,authenticated;
...
-- ============================================================
-- GOOGLE / GMAIL ADMIN SETUP
-- ============================================================
-- After the administrator logs in once with Google, run:
--
...
--
-- The old Telegram ID is no longer required for admin access.


-- ============================================================
-- LOGIN LOOKUP RPCs (used by Gmail/username login)
-- ============================================================
-- Username login lookup: resolve the Auth email from auth.users instead of
-- relying on profiles.auth_email (which may be NULL for older accounts).
...
-- ============================================================
-- 26. FINAL COMPATIBILITY RPCs (dashboard + login)
-- ============================================================
...
-- Keep username login lookup compatible with Gmail based Auth users.
...
-- ============================================================
-- FINAL AUTH/REGISTER HARDENING (safe to run repeatedly)
-- ============================================================
-- The Auth trigger must never create wallets or depend on a unique
-- username. Registration itself owns profile + wallet initialization.
...
-- ============================================================
-- 27. FINAL NON-DESTRUCTIVE AUDIT
-- ============================================================
-- This section ONLY reports objects/conflicts. It does not delete data.
select
  c.relkind,
...
where n.nspname='public'
  and c.relname in (
    'profiles','products','pastelinks','purchases','payments','wallets',
    'wallet_transactions','transactions','withdrawals','approved_bots',
    'admin_logs','product_views','site_stats','notifications',
    'telegram_products','telegram_channels','bot_integrations',
    'marketplace_public'
  )
order by c.relname;

-- Duplicate usernames / Telegram usernames are reported, never deleted.
select lower(username) as duplicate_username, count(*) as total
from public.profiles
...
select lower(telegram_username) as duplicate_telegram_username, count(*) as total
from public.profiles
...
-- Duplicate marketplace/product slugs are reported, never deleted.
select lower(slug) as duplicate_slug, count(*) as total
from public.products
...
-- Duplicate payment order/provider references are reported.
...
-- ============================================================
-- END — SAFE / IDEMPOTENT / NO USER-DATA DROP
-- ============================================================
