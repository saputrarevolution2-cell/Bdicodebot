...
returns table(auth_email text) language sql security definer set search_path=public
as $$ select p.auth_email from public.profiles p where lower(p.username)=lower(trim(p_username)) or lower(p.auth_email)=lower(trim(p_username)) limit 1 $$;
...
create or replace function public.admin_stats() returns setof jsonb language plpgsql security definer set search_path=public as $$begin perform public.assert_admin(); return query select jsonb_build_object('users',(select count(*) from profiles),'products',(select count(*) from products),'orders',(select count(*) from orders),'withdrawals_pending',(select count(*) from withdrawals where status='pending'),'pastes',(select count(*) from pastelinks),'bots_active',(select count(*) from approved_bots where is_active)); end$$;
