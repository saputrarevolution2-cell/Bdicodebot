...
GRANT EXECUTE ON FUNCTION public.resolve_username_login(text) TO anon;
GRANT EXECUTE ON FUNCTION public.resolve_username_login(text) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.add_content_comment(uuid,text,text,text,text)
TO anon,authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_content_comments(uuid,text,integer)
TO anon,authenticated;
...
GRANT EXECUTE ON FUNCTION public.buy_market_item_guest(text,uuid,text)
TO anon,authenticated;
...
-- ============================================================
-- PasTele FINAL PATCH: username login resolver
-- Fixes: 42883 function public.resolve_username_login(text) does not exist
-- Canonical signature used by frontend: resolve_username_login(text)
-- ============================================================
...
-- Keep the resolver from exposing arbitrary profile rows through a broad query.
-- It only returns the canonical login identifier metadata needed by the client.
