-- ============================================================
-- PASTELE / BDICODEBOT
-- CANONICAL DATABASE MASTER — FULL FIX
-- Generated from the supplied project ZIP:
-- Bdicodebot-main (2026-09-17)
--
-- Includes:
--   * supplied canonical database master
--   * Cashi / subscription / wallet / withdrawal schema & RPCs
--   * marketplace schema & RPCs
--   * Paid/Free access protection
--   * guest checkout/access
--   * marketplace public listing
--   * Security Invoker marketplace/profile views
--   * final Paid price contract: Rp2.000–Rp100.000, Rp1.000 steps
--
-- IMPORTANT:
-- Run in Supabase SQL Editor with sufficient privileges.
-- This file preserves the supplied project's existing application schema
-- and applies the supplied security/marketplace patches as final overrides.
-- ============================================================
...
CHECK ((access_type='free' AND price=0) OR (access_type='paid' AND price BETWEEN 2000 AND 100000));
...
BEGIN
 IF NOT public.is_current_user_admin() THEN RAISE EXCEPTION 'ADMIN_REQUIRED'; END IF;
 IF p_price IS NOT NULL AND (p_price<0 OR p_price>100000) THEN RAISE EXCEPTION 'INVALID_PRICE'; END IF;
 CASE src
  WHEN 'products' THEN
...
-- ============================================================
-- FINAL MARKETPLACE PRICE CONTRACT OVERRIDE
-- Paid: Rp2.000 .. Rp100.000, increments Rp1.000.
-- Free: exactly Rp0.
-- ============================================================
BEGIN;

ALTER TABLE public.products
  DROP CONSTRAINT IF EXISTS products_price_access_check;
ALTER TABLE public.products
  ADD CONSTRAINT products_price_access_check
  CHECK (
    (lower(coalesce(access_type,'free'))='free' AND coalesce(price,0)=0)
    OR
    (lower(coalesce(access_type,''))='paid'
      AND price BETWEEN 2000 AND 100000
      AND mod(price,1000)=0)
  );

ALTER TABLE public.telegram_products
  DROP CONSTRAINT IF EXISTS telegram_products_price_access_check;
ALTER TABLE public.telegram_products
  ADD CONSTRAINT telegram_products_price_access_check
  CHECK (
    (lower(coalesce(access_type,'free'))='free' AND coalesce(price,0)=0)
    OR
    (lower(coalesce(access_type,''))='paid'
...
ALTER TABLE public.telegram_channels
  DROP CONSTRAINT IF EXISTS telegram_channels_price_access_check;
ALTER TABLE public.telegram_channels
  ADD CONSTRAINT telegram_channels_price_access_check
  CHECK (
    (lower(coalesce(access_type,'free'))='free' AND coalesce(price,0)=0)
    OR
    (lower(coalesce(access_type,''))='paid'
...
CREATE OR REPLACE FUNCTION public.validate_paid_price(p_price numeric)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT p_price IS NOT NULL
     AND p_price BETWEEN 2000 AND 100000
     AND mod(p_price,1000)=0;
$$;

REVOKE ALL ON FUNCTION public.validate_paid_price(numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.validate_paid_price(numeric)
  TO anon, authenticated;
...
-- ============================================================
-- SUPPLIED PAID ACCESS SECURITY PATCH (FINAL OVERRIDE)
-- ============================================================
...
-- ============================================================
-- SUPPLIED MARKETPLACE CONTENT FIX (FINAL OVERRIDE)
-- ============================================================
...
-- ============================================================
-- SUPPLIED SECURITY INVOKER PATCH (FINAL OVERRIDE)
-- ============================================================
...
-- ============================================================
-- END PASTELE DATABASE MASTER FULL FIX
-- ============================================================


/* ============================================================
PasTele / Bdicodebot
FINAL CONTENT + WALLET + DASHBOARD METRICS PATCH
Run AFTER: PASTELE_DATABASE_MASTER_FULL_FIX_20260917.sql

RULES
- FREE Create: guest + logged-in
- PAID Create: login required
- Views / Likes / Comments / Shares / Sales are readable by pages
- Guest FREE content has no seller account and therefore earns no balance
- Paid content always has an authenticated owner
- Seller earning: 70%
- Platform fee: 30%
- Seller earning is pending until existing H1/H2 settlement releases it
============================================================ */

BEGIN;

-- ============================================================
-- 1. Guest FREE PasteLink support
-- ============================================================
ALTER TABLE public.pastelinks
...
-- 2. Metric indexes
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_content_likes_target_metrics
  ON public.content_likes(target_type,target_id);

CREATE INDEX IF NOT EXISTS idx_content_comments_target_metrics
  ON public.content_comments(target_type,target_id);

CREATE INDEX IF NOT EXISTS idx_analytics_target_event_metrics
  ON public.analytics_events(target_type,target_id,event_type,created_at DESC);

CREATE INDEX IF NOT EXISTS idx_orders_item_paid_metrics
  ON public.orders(item_type,item_id,status);

CREATE INDEX IF NOT EXISTS idx_orders_product_paid_metrics
  ON public.orders(product_id,status);

-- ============================================================
-- 3. Guest likes
--    Guest-created FREE content may have owner_id = NULL.
-- ============================================================
ALTER TABLE public.content_likes
  ALTER COLUMN content_owner_id DROP NOT NULL;
...
BEGIN
  IF p_target_id IS NULL
     OR length(btrim(coalesce(p_guest_token,''))) < 16 THEN
    RAISE EXCEPTION 'INVALID_GUEST_LIKE';
  END IF;
  ...
  IF target IN ('product','link') THEN
    SELECT true,coalesce(creator_id,seller_id)
      INTO target_exists,owner
    FROM public.products
    WHERE id=p_target_id AND status IN ('published','active');

  ELSIF target IN ('code','telegram_product','telegram-product') THEN
    SELECT true,owner_id
      INTO target_exists,owner
    FROM public.telegram_products
    WHERE id=p_target_id AND status IN ('published','active');
  ...
  ELSIF target IN ('pastelink','paste-link') THEN
    SELECT true,user_id
      INTO target_exists,owner
    FROM public.pastelinks
    WHERE id=p_target_id
  ...
  ELSIF target='paste' THEN
    SELECT true,owner_id
      INTO target_exists,owner
    FROM public.pastes
    WHERE id=p_target_id AND visibility='public';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.toggle_content_like_guest(uuid,text,text)
  TO anon,authenticated;

-- ============================================================
-- 4. Guest comments remain allowed
-- ============================================================
DROP POLICY IF EXISTS comments_anon_insert ON public.content_comments;
...
-- ============================================================
-- 5. Unified public metrics view
--
-- This view is intentionally separate from marketplace_public
-- so existing marketplace SELECT contracts are not broken.
--
-- Pages can read:
--   target_id
--   target_type
...
SELECT
  ...
  coalesce(p.sales_count,0)::bigint AS sales,
  (SELECT count(*) FROM public.content_likes l
    WHERE l.target_id=p.id
      AND lower(l.target_type) IN ('product','link'))::bigint AS likes,
  (SELECT count(*) FROM public.content_comments c
    WHERE c.target_id=p.id
      AND lower(c.target_type) IN ('product','link'))::bigint AS comments,
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      AND lower(a.target_type) IN ('product','link')
      AND lower(a.event_type)='share')::bigint AS shares
FROM public.products p
WHERE p.status IN ('published','active')
...
SELECT
  ...
  (SELECT count(*) FROM public.content_likes l
    WHERE l.target_id=p.id
      AND lower(l.target_type) IN ('code','telegram_product','telegram-product'))::bigint,
  (SELECT count(*) FROM public.content_comments c
    WHERE c.target_id=p.id
      AND lower(c.target_type) IN ('code','telegram_product','telegram-product'))::bigint,
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      AND lower(a.target_type) IN ('code','telegram_product','telegram-product')
      AND lower(a.event_type)='share')::bigint
FROM public.telegram_products p
WHERE p.status IN ('published','active')
...
SELECT
  p.id,
  CASE
    WHEN lower(coalesce(p.type,'channel'))='group'
    THEN 'group' ELSE 'channel'
  END::text,
  coalesce(p.views,0)::bigint,
  coalesce(p.sales_count,0)::bigint,
  (SELECT count(*) FROM public.content_likes l
    WHERE l.target_id=p.id
      AND lower(l.target_type) IN (
        'channel','group','telegram_channel','telegram-channel',
        'telegram_group','telegram-group'))::bigint,
  (SELECT count(*) FROM public.content_comments c
    WHERE c.target_id=p.id
      AND lower(c.target_type) IN (
        'channel','group','telegram_channel','telegram-channel',
        'telegram_group','telegram-group'))::bigint,
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      AND lower(a.target_type) IN (
        'channel','group','telegram_channel','telegram-channel',
        'telegram_group','telegram-group')
      AND lower(a.event_type)='share')::bigint
FROM public.telegram_channels p
...
SELECT
  ...
  (SELECT count(*) FROM public.content_likes l
    WHERE l.target_id=p.id
      AND lower(l.target_type) IN ('pastelink','paste-link'))::bigint,
  (SELECT count(*) FROM public.content_comments c
    WHERE c.target_id=p.id
      AND lower(c.target_type) IN ('pastelink','paste-link'))::bigint,
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      AND lower(a.target_type) IN ('pastelink','paste-link')
      AND lower(a.event_type)='share')::bigint
FROM public.pastelinks p
WHERE p.visibility='public'
  AND (p.expires_at IS NULL OR p.expires_at>now())
...
SELECT
  ...
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      ...
      AND lower(a.event_type)='view')::bigint,
  0::bigint,
  (SELECT count(*) FROM public.content_likes l
    WHERE l.target_id=p.id AND lower(l.target_type)='paste')::bigint,
  (SELECT count(*) FROM public.content_comments c
    WHERE c.target_id=p.id AND lower(c.target_type)='paste')::bigint,
  (SELECT count(*) FROM public.analytics_events a
    WHERE a.target_id=p.id
      AND lower(a.target_type)='paste'
      AND lower(a.event_type)='share')::bigint
...
-- ============================================================
-- 8. Keep PasteLink sales_count synchronized with completed sales
-- ============================================================
CREATE OR REPLACE FUNCTION public.sync_pastelink_sale_counter()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
AS $$
BEGIN
  IF lower(coalesce(NEW.item_type,'')) IN ('pastelink','paste-link')
     AND lower(coalesce(NEW.status,'')) IN ('completed','paid','success','settled')
...
DROP TRIGGER IF EXISTS trg_sync_pastelink_sale_counter
  ON public.purchases;

CREATE TRIGGER trg_sync_pastelink_sale_counter
AFTER INSERT ON public.purchases
FOR EACH ROW
EXECUTE FUNCTION public.sync_pastelink_sale_counter();

-- ============================================================
-- 9. Backfill PasteLink sales counter from existing paid orders.
-- ============================================================
UPDATE public.pastelinks p
SET sales_count=(
  SELECT count(*)
  FROM public.orders o
  WHERE lower(coalesce(o.item_type,'')) IN ('pastelink','paste-link')
    AND o.item_id=p.id::text
...
-- ============================================================
-- 10. Verify FREE / PAID creation contracts.
--     Creation functions already enforce:
--       FREE -> auth.uid() may be NULL
--       PAID -> LOGIN_REQUIRED_FOR_PAID
-- ============================================================
...
GRANT EXECUTE ON FUNCTION public.create_telegram_content(
...
-- Active public metrics:
SELECT *
FROM public.content_metrics_public
ORDER BY target_type,target_id
LIMIT 50;

-- Check creator dashboard (run while logged in):
-- SELECT public.get_creator_dashboard_stats();

-- Check pending seller settlement:
-- SELECT * FROM public.get_pending_balance_detail();

-- Check wallet:
-- SELECT * FROM public.wallets
-- WHERE user_id=auth.uid();

-- Check PasteLink guest-owner compatibility:
SELECT column_name,is_nullable,data_type
FROM information_schema.columns
WHERE table_schema='public'
  AND table_name='pastelinks'
  AND column_name IN ('user_id','sales_count');
